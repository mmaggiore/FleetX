...
approve = valid8(trim(request.querystring("action")))

if approve = "a" then
  bt_id = valid8(trim(request.querystring("id")))
      Set oConn = Server.CreateObject("ADODB.Connection")
      oConn.ConnectionTimeout = 100
      oConn.Provider = "MSDASQL"
      oConn.Open DATABASE
        'update 
        SQL="UPDATE fcbillto set bt_Status='c' WHERE bt_ID=" & bt_ID
...
SQL="SELECT * FROM PreExistingRequestor WHERE (RequestorStatus='n')"
...
%>
<table align="center" width="65%" class="MainPageText"><tr><td><b>Name</b></td><td><b>Email</b></td><td><b>City</b></td><td><b>State</b></td><td><b>action</b></td></tr>
<%
    ...
    %>
        <tr><td><%=Recordset1("RequestorName")%></td><td><%=Recordset1("RequestorEmail")%></td><td><%=Recordset1("RequestorCity")%></td><td><%=Recordset1("RequestorState")%></td><td><a href="NewUserReview.asp?id=<%=Recordset1("RequestorID")%>" class="FleetXRedMain">edit/review</a></td></tr>
    <%
    Recordset1.MoveNext
    Loop
	Else
