...
if Edit = "Y" then
  ...
  if len(valid8(trim(request.form("atBillCode")))) < 1 then
    ErrorUpdate = "You must enter a Bill Code<br>"
     ...
     SQL = "SELECT * from AccessorialType where atBillCode = '" & valid8(request.form("atBillCode")) & "' and atid <> " & ID
  ...
  If len(ErrorUpdate) < 1 then
      SQL="UPDATE AccessorialType set atBillCode='" & valid8(request.form("atBillCode")) & "', atDescr = '" & valid8(request.form("atDescr")) & "' WHERE atID=" & ID
...
oConn.Provider = "MSDASQL"
oConn.Open DATABASE
SQL="SELECT * from AccessorialType WHERE atID=" & ID
SET oRs = oConn.Execute(SQL)
if NOT oRs.EOF then
  atBillCode = oRs("atBillCode")
  atDescr = oRs("atDescr")
else
...
Set oRS=Nothing
    ...
    CurrentDateTime=Now()
    PageTitle="ACCESSORIAL TYPE EDIT"

%>
<title>FleetX - <%=PageTitle %></title>
</head>

<body onload="document.EditType.atBillCodefocus();">
    ...
    <tr><td colspan="2">
<form action="AccessorialTypeEdit.asp" method="post" name="EditType">
...
<% if len(ErrorMessage) < 1 then %>
  ...
  <% end if %>
  <tr><td><b>Bill Code</b></td><td><input type=text name="atBillCode" value="<%=atBillCode%>"></td></tr>
  <tr><td><b>Description</b></td><td><input type="text" name="atDescr" value="<%=atDescr%>"></td></tr>
  <input type="hidden" name="atType" value="<%=atType%>">
...
<tr><td colspan=6>&nbsp;<br><br><a href="AccessorialTypes.asp" class="FleetXRedMain">CLICK HERE</a> to Return to Accessorial Types</td></tr>
