...
'''''''Shows all previous billings and allows you to export to XLS for billings
    ...
    TypeOfOrder= valid8(request.form("TypeOfOrder"))
If GoButton="GO" then
    ...
    Select Case TypeOfOrder
        Case "Courier"
            ExtraCriteria=" AND fh_bt_id IN('92') "
        Case "Hotshot"
            ExtraCriteria=" AND fh_bt_id IN('93') "
        Case "Stockroom"
            ExtraCriteria=" AND fh_bt_id IN('91') "
        Case else
            ExtraCriteria=" AND fh_bt_id IN('91','92','93') "
    End Select
        ...
        End if
				...
				oConn.Open DATABASE
					'l_cSQL2 = "select fl_fh_id FROM fcfgthd INNER JOIN fclegs ON fcfgthd.fh_id = fclegs.fl_fh_id INNER JOIN PreExistingCompanies ON fclegs.fl_sf_id = PreExistingCompanies.ST_ID INNER JOIN PreExistingCompanies PreExistingCompanies_1 ON fclegs.fl_st_id = PreExistingCompanies_1.ST_ID WHERE fl_t_atd>'"&StartDate&"' and fl_t_atd<'"&EndDate&"' and fh_status='CLS' and (fh_billed_date IS NULL) and (PreExistingCompanies_1.CarillonID > 10) AND (PreExistingCompanies.CarillonID > 10) "&ExtraCriteria 
					l_cSQL2 = "select fl_fh_id FROM fcfgthd INNER JOIN fclegs ON fcfgthd.fh_id = fclegs.fl_fh_id INNER JOIN PreExistingCompanies ON fclegs.fl_sf_id = PreExistingCompanies.ST_ID INNER JOIN PreExistingCompanies PreExistingCompanies_1 ON fclegs.fl_st_id = PreExistingCompanies_1.ST_ID WHERE fl_t_atd>'"&StartDate&"' and fl_t_atd<'"&EndDate&"' and fh_status='CLS' and (PreExistingCompanies_1.CarillonID > 10) AND (PreExistingCompanies.CarillonID > 10) "&ExtraCriteria
...
If QQQ<1 then QQQ=0
	Set RSEVENTS2 = Server.CreateObject("ADODB.Recordset")
        ...
        RSEVENTS2("BillingType")=TypeOfOrder
...
SQL = SQL&"PreviousTIBillings.BillingType, PreviousTIBillings.NumberOfJobs, PreviousTIBillings.UserID, PreviousTIBillings.BillingStatus, PreExistingRequestor.RequestorName "
SQL = SQL&"FROM PreviousTIBillings INNER JOIN PreExistingRequestor ON PreviousTIBillings.UserID = PreExistingRequestor.RequestorID Where BillingStatus='c' order by BillingDate desc"
...
<Table border="0" cellpadding="0" cellspacing="0">
    <tr><td>Select Type of Orders:</td><td><input type="radio" name="TypeOfOrder" value="Courier" <%If TypeOfOrder="Courier" then response.write " checked" end if %> /> Courier </td></tr>
<tr><td>&nbsp</td><td>  <input type="radio" name="TypeOfOrder" value="Hotshot"  <%If TypeOfOrder="Hotshot" then response.write " checked" end if %>   /> Hotshot </td></tr>
<tr><td>&nbsp</td><td>    <input type="radio" name="TypeOfOrder" value="Stockroom" <%If TypeOfOrder="Stockroom" then response.write " checked" end if %> /> Stockroom </td></tr>
<tr><td>&nbsp</td> <td>     <input type="radio" name="TypeOfOrder" value="All" <%If TypeOfOrder="All" or TypeOfOrder="" then response.write " checked" end if %>/> All</td></tr>
</table>
...
<%end if%>
<tr><td>&nbsp;</td><td  nowrap="nowrap" ><b>Billing Date</b></td><td><b>From</b></td><td><b>To</b></td><td  nowrap="nowrap" ><b>Type</b></td><td  nowrap="nowrap" ><b>Jobs</b></td><td  nowrap="nowrap" ><b>Billed by</b></td></tr>
<%
    ...
    Do Until Recordset1.EOF
        ...
        BillingType=Recordset1("BillingType")
        NumberOfJobs=Recordset1("NumberOfJobs")
        RequestorName=Recordset1("RequestorName")
        'response.write "BillingDate="&BillingDate&"<BR>"
        'response.write "BilledDate="&BilledDate&"<BR>"
        If BillingDate=BilledDate then
            FontVar="blue"
            'response.write "got here blue<br>"
            else
            FontVar="black"
            'response.write "got here black<br>"
        End if

    %>
    <form method="post" action="CreateBillingSpreadsheet.asp" target="_blank">
        <tr><td><INPUT id="gobutton" name="gobutton" TYPE="submit" name="ButtonValue" VALUE="EXPORT"></td><td  nowrap="nowrap" ><font color="<%=FontVar %>"><%=BillingDate%></font></td><td><font color="<%=FontVar %>"><%=BillingStartDate%></font></td><td nowrap="nowrap"><font color="<%=FontVar %>"><%=BillingEndDate%></font></td><td  nowrap="nowrap" ><font color="<%=FontVar %>"><%=BillingType%></font></td><td  nowrap="nowrap" ><font color="<%=FontVar %>"><%=NumberOfJobs%></font></td><td  nowrap="nowrap" ><font color="<%=FontVar %>"><%=RequestorName%></font></td></tr></font>
