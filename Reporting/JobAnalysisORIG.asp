...
l_cSQL=l_cSQL&"Select * from marksview2 "
...
Set oRs2 = oConn2.Execute(l_cSQL2)
	...
	LOOP
Set oRs2=nothing
'Response.write "Reflist="&Reflist&"XX<BR>"
...
oConn2.Open DATABASE
Err.Clear
l_cSQL2="SELECT st_addr1 AS Address1, st_addr2 AS Address2, st_city AS City, st_state AS State, "
l_cSQL2=l_cSQL2&" st_country AS Country, st_zip AS Zip FROM fcshipto " 
l_cSQL2=l_cSQL2&" WHERE (st_id= '"&ToLocation&"')"					
Set oRs2 = oConn2.Execute(l_cSQL2)
If not oRs2.eof then
ToAddress1=oRs2("Address1")
ToAddress2=oRs2("Address2")
ToCity=oRs2("City")
ToState=oRs2("State")
ToZip=oRs2("Zip")
ToCountry=oRs2("Country")
End if
Set oRs2=nothing
...
FromAddress2=oRs2("Address2")
FromCity=oRs2("City")
FromState=oRs2("State")
FromZip=oRs2("Zip")
FromCountry=oRs2("Country")
End if
Set oRs2=nothing
...
<%
	...
	If trim(BillToID)="48" then
		'Response.Write "WHATEVER!!!<br>"
			...
			'response.write "!!!l_cSQL48="&l_cSQL48&"<BR>"
			'response.write "!!!Database="&Database&"<BR>"
			'response.write "!!!BillToID="&BillToID&"<BR>"
