...
<!-- #include file="../Genericfleetexpress.inc" -->
<%
...
SubmittedvarCaptcha=valid8(Request.Form("varCaptcha"))
...
If ResetButton<>"clear search" then
	DateSentFrom=valid8(Request.Form("DateSentFrom"))
	DateSentTo=valid8(Request.Form("DateSentTo"))
	DocumentNumber=valid8(Request.Form("DocumentNumber"))
	If DocumentNumber="" then
		DocumentNumber=valid8(Request.QueryString("DocumentNumber"))
	End if
	LotNumber=valid8(Request.Form("LotNumber"))
	If LotNumber="" then
		LotNumber=valid8(Request.QueryString("LotNumber"))
	End if
	SortBy=valid8(Request.Form("SortBy"))
	ToLocation=valid8(Request.Form("ToLocation"))
	FromLocation=valid8(Request.Form("FromLocation"))
    RequestorEmail=valid8(Request.Form("RequestorEmail"))
	JobNumber=valid8(Request.Form("JobNumber"))
    DocumentNumber=valid8(Request.form("DocumentNumber"))
	If JobNumber="" then
		JobNumber=valid8(Request.QueryString("JobNumber"))
	End if
	ReferenceNumber=valid8(Request.Form("ReferenceNumber"))
	Priority=valid8(Request.Form("Priority"))
	JobStatus=valid8(Request.Form("JobStatus"))
End if
    ...
    <tr><td width="100%" align="center" valign="top">
	 ...
	 <TABLE WIDTH="100%" valign="top" border="0" bordercolor="red" cellpadding="0" cellspacing="5" ID="Table1">
		...
		<table cellpadding="0" valign="top" cellspacing="0" border="0" bordercolor="brown" align="center" ID="Table2">  
			<tr><td class="MainPageText">To track an order:<br />1. Provide a complete order number, complete document number, or requestor email. 
            <br />2.  Enter the verification code.<br />3. Click "SEARCH"</td></tr>
			<tr><td>&nbsp;</td></tr>
            <tr><td>&nbsp;</td></tr>
			<tr>
				<td>
					<table border="0" bordercolor="red" align="center" ID="Table3" class="MainPageText">
					<form method="post" action="GenericTracking.asp" name="thisForm" ID="Form1">
				...
				<!--		<tr>
                        ...
                        %>
 						<tr>
							...
							<td>
								<input type="text" name="ReferenceNumber" value="<%=ReferenceNumber%>" size="20" ID="Text4">
							</td>					
						</tr> 
 						<tr>
							<td class="MainPageText" align="right">
							...
							<td>
								<input type="text" name="RequestorEmail" value="<%=RequestorEmail%>" size="20" ID="Text1">
							</td>					
						</tr> 
<%
							...
							<td>
								<select name="Priority" ID="Select4">
									...
									Set RSEVENTS2 = Server.CreateObject("ADODB.Recordset")
										...
										l_csql = "SELECT DISTINCT PriorityDescription AS PriorityDescription, PriorityMinutes AS PriorityTime, Priority AS PriorityAbbreviation FROM priorities order by Priority"
							...
							<td>
								<select name="FromLocation" ID="Select2">
									...
									Set RSEVENTS2 = Server.CreateObject("ADODB.Recordset")
										...
										l_csql = "SELECT * FROM PreExistingCompanies WHERE (CompanyStatus = 'c') AND (CompanyOwner = '"& UserID &"')"
                                        If BilltoID=3699 then
										    l_csql=l_csql&" OR sb_st_id='TISHERMA' "
										End if										
										If BillToID=4899 then
											l_csql=l_csql&" AND (St_Priapt='DFW')"
										End if
										If BillToID=3899 then
											l_csql=l_csql&" AND (st_id<>'CPGP')"
										End if
										If BillToID=7699 then
											l_csql=l_csql&" OR (st_id='TOPPAN')"
										End if												
										l_csql=l_csql&" ORDER BY contactname, st_id"										
										'response.write("Query2:" & l_cSQL)
										
										RSEVENTS2.Open l_cSQL, Database, 1, 3
										Do while not RSEVENTS2.EOF
                                        st_id=RSEVENTS2("st_id")
										CompanyName=RSEVENTS2("CompanyName")
										CompanyAddress=RSEVENTS2("CompanyAddress")
                                        CompanyBuilding=RSEVENTS2("CompanyBuilding")
                                        CompanySuite=RSEVENTS2("CompanySuite")
                                        CompanyCity=RSEVENTS2("CompanyCity")
                                        ContactName=RSEVENTS2("ContactName")
                                        IsStockroom=RSEVENTS2("IsStockroom")
										%>
										
											<option value="<%=st_id%>" <%if st_id=FromLocation then response.Write " Selected " end if%>><%If IsStockroom<>"y" then response.write ContactName&"/" end if%><%=CompanyName%><% If IsStockroom<>"y" then response.write "/"&CompanyAddress&"/"&CompanyBuilding&"/"&CompanySuite&"/"&CompanyCity&"/"&ContactName End if%></option>
								...
								<select name="ToLocation" ID="Select1">
									...
									Set RSEVENTS2 = Server.CreateObject("ADODB.Recordset")
										...
										%>
											...
											<!--option value="<%=sb_st_id%>" <%if st_id=ToLocation then response.Write " Selected " end if%>><%=ContactName%>/<%=CompanyName%>/<%=CompanyAddress%>/<%=CompanyBuilding%>/<%=CompanySuite%>/<%=CompanyCity%>/<%=ContactName%></option-->
										    <option value="<%=sb_st_id%>" <%if st_id=ToLocation then response.Write " Selected " end if%>><%If IsStockroom<>"y" then response.write ContactName&"/" end if%><%=CompanyName%><% If IsStockroom<>"y" then response.write "/"&CompanyAddress&"/"&CompanyBuilding&"/"&CompanySuite&"/"&CompanyCity&"/"&ContactName End if%></option>
										<%	
										RSEVENTS2.movenext
										LOOP
										RSEVENTS2.close
...
<%end if %>
		...
		<%
            ...
            'Response.write "SubmittedvarCaptcha="&SubmittedvarCaptcha&"<BR>"

    If CaptchaSubmit<>SubmittedvarCaptcha then
        ErrorMessage="You did not supply the correct verification code"
    End if
If trim(jobnumber)="" and trim(ReferenceNumber)="" and trim(RequestorEmail)="" then
        ErrorMessage="You must provide an order number, a document number, or a requestor email"
End if
...
If trim(errormessage)="" then
			Set RSEVENTS2 = Server.CreateObject("ADODB.Recordset")
				...
				'Response.write "NumberofDays="&NumberofDays&"<BR>"
					...
					l_csql = "SELECT top 300 fcfgthd.fh_id, fclegs.fl_sf_rta, fcfgthd.fh_status, fh_bt_id, fcfgthd.fh_ship_dt, fclegs.fl_sf_id, fclegs.fl_sf_name, fclegs.fl_sf_clname, fclegs.fl_sf_addr1, fclegs.fl_sf_addr2, fclegs.fl_st_id, fclegs.fl_st_name, fclegs.fl_st_clname, fclegs.fl_st_addr1, fclegs.fl_st_addr2, fclegs.fl_t_atp, fclegs.fl_t_acc, fclegs.fl_t_atd, fclegs.fl_pod, fcfgthd.fh_priority FROM fcfgthd INNER JOIN fclegs ON fcfgthd.fh_id = fclegs.fl_fh_id "	
        ...
        ''''End if
				...
				'End Select
                        ...
                        If trim(ReferenceNumber)>"" then
                            l_csql = L_csql& "AND (fh_custpo = '"&ReferenceNumber&"') "
                        End if							
                        If trim(RequestorEmail)>"" then
                            l_csql = L_csql& "AND ((fh_RequestorEmail = '"&RequestorEmail&"') OR (fh_co_email = '"&RequestorEmail&"')) "
                        End if						
                        
                        											
						'If DateSentTo>"" And DateSentFrom>"" and trim(LotNumber)="" and trim(JobNumber)="" then
                        '''If DateSentTo>"" And DateSentFrom>"" and trim(JobNumber)="" then
						'''	If BillToID=26 then
						...
						If trim(LotNumber)>"" and trim(JobNumber)="" then
							l_csql = L_csql& "AND (rf_ref = '"&trim(LotNumber)&"') "
						End if
						If trim(JobNumber)>"" AND trim(LotNumber)="" then
							l_csql = L_csql& "AND (fh_id = '"&trim(JobNumber)&"') "
						End if	
						If trim(JobNumber)>"" AND trim(LotNumber)>"" then
							l_csql = L_csql& "AND ((rf_ref = '"&trim(LotNumber)&"') AND (fh_id = '"&trim(JobNumber)&"')) "
						End if	
			...
			%>
				<tr>
 						        ...
 						        <td class="MainPageTextSmaller" nowrap valign="top" bgcolor="<%=colorset(i mod numcolors)%>">
							        <a href="../reporting/GenericOrderDetails.asp?inputlotnumber=<%=trim(rf_ref)%>&inputjobnumber=<%=fh_id%>"><%=fh_id%></a>
						        </td>				
                    ...
                    If fh_bt_id="91" then
                        ...
                        If trim(fl_sf_addr2)>"" then
                            fl_sf_name=fl_sf_clname&"/"&fl_sf_addr2
                            else
                            fl_sf_name=fl_sf_clname&"/"&fl_sf_addr1
                        End if
                        If trim(fl_st_addr2)>"" then
                            fl_st_name=fl_st_clname&"/"&fl_st_addr2
                            else
                            fl_st_name=fl_st_clname&"/"&fl_st_addr1
                        End if
				...
				If i>1 then
					...
					Select Case BillToID
						...
						Case 36
							...
							Response.Redirect("../reporting/GenericOrderDetails.asp?inputlotnumber="&trim(rf_ref)&"&inputjobnumber="&fh_id)
						Case 38
							...
							Response.Redirect("../reporting/GenericOrderDetails.asp?inputjobnumber="&fh_id)																								
						Case 48, 13, 14, 25
