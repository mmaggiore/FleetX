	 ...
	 <TABLE WIDTH="100%" valign="top" border="0" bordercolor="red" cellpadding="0" cellspacing="5" ID="Table1">
		...
		<table cellpadding="0" valign="top" cellspacing="0" border="0" bordercolor="brown" align="center" ID="Table2">  
			...
			<tr>
				...
				<!--		<tr>
								...
								<select name="FromLocation" ID="Select2">
									...
									Set RSEVENTS2 = Server.CreateObject("ADODB.Recordset")
										...
										%>
											...
											<option value="<%=st_id%>" <%if st_id=FromLocation then response.Write " Selected " end if%>><%If (IsStockroom<>"y" or isNULL(IsStockroom)) then response.write ContactName&"/" end if%><%=CompanyName%><% If (IsStockroom<>"y" or isNULL(IsStockroom)) then response.write "/"&CompanyAddress&"/"&CompanyBuilding&"/"&CompanySuite&"/"&CompanyCity End if%></option>
								...
								<select name="ToLocation" ID="Select1">
									...
									Set RSEVENTS2 = Server.CreateObject("ADODB.Recordset")
										...
										%>
										    ...
										    <option value="<%=st_id%>" <%if st_id=ToLocation then response.Write " Selected " end if%>><%If (IsStockroom<>"y" or isNULL(IsStockroom)) then response.write ContactName&"/" end if%><%=CompanyName%><% If (IsStockroom<>"y" or isNULL(IsStockroom)) then response.write "/"&CompanyAddress&"/"&CompanyBuilding&"/"&CompanySuite&"/"&CompanyCity End if%></option>
										<%	
										RSEVENTS2.movenext
										LOOP
										RSEVENTS2.close
                        ...
                        <input type="submit" name="submit" value="search" id="gobutton">
                        </form>
                        </td><td>&nbsp;</td><td valign="top">
                  <form method="post" action="tracking.asp">
                    <input type="submit" id="gobutton" name="submit" value="clear" />
        ...
        ''''End if
				...
				'End Select
						...
						End if	
 						If trim(LotNumber)="" and trim(JobNumber)="" and trim(DocumentNumber)="" then
							l_csql = L_csql& "AND (fh_user_id = '"&trim(UserID)&"') "
						End if                       						
