...
<%
    ...
    CurrentDateTime=Now()
    PageTitle="FLEET EXPRESS ORDER PAGE"
...
<title>FleetX - <%=PageTitle %></title>
   ...
   EstimatedCharge=Request.Form("EstimatedCharge")
   if isNull(EstimatedCharge) or NOT isnumeric(EstimatedCharge)  then
    EstimatedCharge = 0
   end if
    ColorSelect=Request.form("ColorSelect")
    ColorSelect=ColorSelect+1
   ...
   FXCourieruserid=Request.cookies("FleetXCookie")("UserID")
   'response.write "78 courieruser=" & FXCourieruserid & "<br>"

''''''''CHECKS TO SEE IF A SUPERVISOR
Set Recordset1 = Server.CreateObject("ADODB.Recordset")
'Response.write "Database="&Database&"<br>"
Recordset1.ActiveConnection = Database
SQL="SELECT * FROM PreExistingRequestor WHERE (RequestorID='"&FXCourierUserID&"') AND (RequestorStatus='c')"
...
'response.write "<font color='red'>SQL1="&Recordset1.Source&"<BR></font>"
	if NOT Recordset1.EOF then
        ...
        DispUserFirstName=Recordset1("RequestorName")
  else
        Supervisor="n"
...
'response.write   "FXCourieruserid="&FXCourieruserid&"<BR>"
 ...
 'response.write "105 here<br>"
   ...
   If trim(UserID)="" and xxx="xxx" then
    ...
    End if
    UserID = FXCourierUserID
   LogInVerified=Request.form("LogInVerified")
   'response.write "114 FXCourierUserID="&FXCourierUserID&"<BR>"
...
<script type="text/javascript">
    ...
    Recordset1.Source = "SELECT * FROM PreExistingRequestor WHERE (RequestorID='"&FXCourierUserID&"') AND (RequestorStatus='c')"
...
''''''''''''''MOVED ERROR MESSAGES FROM HERE!
        ...
        If trim(PreExistingRequestor)>"" then
		    Set RSEVENTS = Server.CreateObject("ADODB.Recordset")
		    RSEVENTS.Open "PreExistingRequestor", Database, 2, 2
		    RSEVENTS.Find "RequestorID='" & PreExistingRequestor & "'"
			    RSEVENTS("RequestorName")=RequestorFirstName
			    'RSEVENTS("Userlastname")=Requestorlastname
			    RSEVENTS("RequestorAddress")=RequestorAddress
			    RSEVENTS("RequestorCity")=RequestorCity
			    'RSEVENTS("RequestorState")=RequestorState
			    RSEVENTS("RequestorZipCode")=RequestorZipCode
			    RSEVENTS("RequestorPhone")=RequestorPhoneNumber
			    'RSEVENTS("UserPhoneCarrier")=CelCarrier
			    RSEVENTS("RequestorEmail")=RequestorEmailAddress
			    RSEVENTS("RequestorPassword")=RequestorPassword
			    'RSEVENTS("DeliveryNotifications")=DeliveryNotifications
		    RSEVENTS.update
		    RSEVENTS.close
		    set RSEVENTS = nothing
        ...
        End if
			...
			Set RSEVENTS2 = Server.CreateObject("ADODB.Recordset")
                ...
                RSEVENTS2("fh_co_id")=Trim(RequestorFirstName)
                ...
                RSEVENTS2("fh_ShipCost")=EstimatedCharge
  ...
  ''''''''''''''''LETTER to Requestor'''''''''''''''''''
                    ...
                    Body = Body & "If a barcode is not visible below, then please <a href='http://www.logisticorp.us/intranet/FleetX/orderentry/FleetXFreightOrderConfirmation.asp?jid="&newjobnum&"'>click here</a> for a printable waybill. <br><br>"
                    ...
                    Response.Redirect("FleetXFreightOrderConfirmation.asp?x=1&y=1&bid=86&pid=view&jid="& newjobnum &"&Internal="&Internal&"&VarA="&FXCourieruserid&"&VarB="&Supervisor)	
         else
...
<table border="0" bordercolor="green" Cellspacing="0" Cellpadding="0" align="left" width="100%">
 <tr><td align="left" colspan="2" bgcolor="<%=HeaderBorderColor%>" class="FleetXRedSection"><img src="../images/pixel.gif" height="5" width="1" /></td></tr>

        <tr><td align="center" bgcolor="<%=HeaderBorderColor%>" class="FleetXRedSection" colspan="2"><%=PageTitle%></td></tr>

        <tr><td align="center" bgcolor="<%=HeaderBorderColor%>"class="FleetXRedSection" colspan="2"><img src="../images/pixel.gif" height="5" width="1" /></td></tr>
...
<form method="post" name="OrderForm1" action="FleetXOrder.asp?Internal=<%=Internal%>">
    <table border="0" cellpadding="0" cellspacing="0" align="center">
        ...
        <tr>
            <%somenumber=13
            'response.write "1403 supervisor=" & supervisor & "<br>"
            If Supervisor<>"y" or isNULL(Supervisor) then
            ...
            If Trim(Supervisor)="y" then
                ...
                <td align="left">
                                ...
                                <%
									...
									oConn.Provider = "MSDASQL"
									oConn.Open DATABASE
										l_cSQL = "Select * FROM PreExistingRequestor WHERE Requestorstatus='c' and RequestorType is NULL ORDER BY RequestorName"
										'response.write "l_cSQL="&l_cSQL&"<BR>"
                                        SET oRs = oConn.Execute(l_cSql)
												Do While not oRs.EOF
                                                supertempRequestorUserID=trim(oRs("RequestorID"))
												SupertempRequestorName=trim(oRs("RequestorName"))
                                                ...
                                                'If trim(Preexistingrequestor)>"" then
                                                    ...
                                                    tempRequestorUserID=trim(oRs("RequestorID"))
												    tempRequestorFirstName=trim(oRs("RequestorName"))
							                        'tempRequestorLastName=trim(oRs("UserLastName"))                                                   
                                                    tempRequestorAddress=trim(oRs("RequestorAddress"))
                                                    tempRequestorCity=trim(oRs("RequestorCity"))
												    tempRequestorState=trim(oRs("RequestorState"))
							                        tempRequestorZipCode=trim(oRs("RequestorZipCode"))
                                                    tempRequestorPhoneNumber=trim(oRs("RequestorPhone"))
                                                    'tempRequestorPhoneCarrier=trim(oRs("UserPhoneCarrier"))
												    tempRequestorEmailAddress=trim(oRs("RequestorEmail"))
							                        tempRequestorUsername=trim(oRs("RequestorEmail"))
                                                    tempRequestorPassword=trim(oRs("RequestorPassword"))
                                                    'tempRequestorDeliveryNotifications=trim(oRs("DeliveryNotifications"))
												    tempRequestorSupervisor=trim(oRs("RequestorType"))
                                if tempRequestorSupervisor = "A" then
                                  tempRequestorSupervisor = "y"
                                else
                                  tempRequestorSupervisor = "n"
                                end if
							                        tempRequestorUserStatus=trim(oRs("RequestorStatus"))
...
<%
									...
									oConn.Provider = "MSDASQL"
									oConn.Open DATABASE
										l_cSQL = "Select * FROM RateList WHERE Ratestatus='c' and RateType='F'"
										SET oRs = oConn.Execute(l_cSql)
												If not oRs.EOF then
                            RateCharge=oRs("RateCharge")
                                                ...
                                                End if								
									    Set oConn=Nothing
                                        EstimatedCharge=Round(ccur(RateCharge), 2)
