		...
		<table cellpadding="0" cellspacing="0" border="0" bordercolor="blue" align="center" ID="Table3">
			...
			<tr>
				<td>
					...
					<form method="post" action="CancelPage.asp" name="thisForm" ID="Form1">
							...
							<td>
								<select name="FromLocation" ID="Select2">
									...
									Set RSEVENTS2 = Server.CreateObject("ADODB.Recordset")
										...
										RSEVENTS2.CursorType = 3
										RSEVENTS2.ActiveConnection = Database
										l_csql = "SELECT companyname as st_name, st_id FROM PreExistingCompanies WHERE (IsStockroom='y')"
										If BilltoID=36 then
										...
										%>
											...
											<option value="<%=st_id%>" <%if st_id=FromLocation then response.Write " Selected " end if%>><%=st_name%></option>
								...
								<select name="ToLocation" ID="Select1">
									...
									Set RSEVENTS2 = Server.CreateObject("ADODB.Recordset")
										...
										%>
											...
											<option value="<%=st_id%>" <%if st_id=ToLocation then response.Write " Selected " end if%>><%=st_name%></option>
		...
		<%
			...
			Set RSEVENTS2 = Server.CreateObject("ADODB.Recordset")
				...
				'Response.write "NumberofDays="&NumberofDays&"<BR>"
					...
					If NumberofDays>0 then
						l_csql = l_csql&" Top 50 "	
					End if		
					l_csql = l_csql&" fh_id, fh_status, fh_ship_dt, fl_sf_id, fl_sf_name, fl_st_id, fl_st_name, fl_t_acc, fl_t_atp, fl_t_atd, fh_custpo, fh_priority, RF_REF FROM fcfgthd INNER JOIN fclegs ON fcfgthd.fh_id = fclegs.fl_fh_id INNER JOIN fcrefs ON fclegs.fl_fh_id = fcrefs.rf_fh_id "	
				...
				Select Case BillToName
					...
					Case else
                        ...
                        l_csql = l_csql&" WHERE ((fh_bt_id='91') "
				End Select
						...
						'''If trim(JobStatus)>"" then
							'''Select Case JobStatus
								...
								'''Case "OPEN"
									l_csql = L_csql& " AND ((fh_status = 'RAP') or (fh_status = 'OPN') or (fh_status = 'ACC')) "
							'''End Select
			...
			''''''''''''''''''''''''''''''''''''''''
				...
				If RSEVENTS2.eof then
						ErrorMessage="Based on your provided criteria, no jobs were found that are available to cancel.<br><br>Please check your criteria and try again.<br><br>Once a job has already been on boarded, then it cannot be cancelled.<br><br>If you need assistance with an order, contact a LogistiCorp supervisor:<br><br>David Mercer 214-882-1292<br>Keith Chitwood 214-882-5423"	
				End if				
				Do while not RSEVENTS2.EOF 
			...
			%>
				<tr>
                    <form action="cancelpage.asp" method="post">
