...
if valid8(request.querystring("action")) = "logout" then
  vehicleID = valid8(request.querystring("ID"))
  'response.write "VehicleID="&VehicleID&"<BR>"
    ...
    SQL = "UPDATE AvailableVehicles SET LogoutTime='" & Now() & "', AvailableStatus='x' WHERE VehicleID = " & clng(vehicleID)
...
SQL = "select * from AvailableVehicles where DriverID IS NOT NULL and LoginTime IS NOT NULL and AvailableStatus = 'c'"
...
%>
<table align="center" width="85%" class="MainPageText"><tr><td><b>Vehicle</b></td><td><b>Vehicle Type</b></td><td><b>Driver</b></td><td><b>Login Time</b></td><td><b>action</b></td></tr>
<%                                                                                                                        
    ...
    Do Until Recordset1.EOF
      ...
      iuSQL = "Select FirstName, LastName FROM intranet_users WHERE UserID = " & Recordset1("DriverID")
    ...
    %>
        <tr><td><%=Recordset1("VehicleName")%></td><td><%=Recordset1("VehicleType")%></td><td>(<%=Recordset1("DriverID")%>) <%=DriverName%></td><td><%=Recordset1("LoginTime")%></td><td><a href="LogoutDrivers.asp?id=<%=Recordset1("VehicleID")%>&action=logout" class="FleetXRedMain">logout driver</a></td></tr>
    <%
    Recordset1.MoveNext
    Loop
	Else
      %><tr><td colspan=6>NO AVAILABLE VEHICLES FOUND</td></tr><%
  End if
Recordset1.Close()
