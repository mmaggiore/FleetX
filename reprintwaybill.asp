...
<%
    ...
    CurrentDateTime=Now()
    PageTitle="REPRINT WAYBILL"

%>
<title>FleetX - <%=PageTitle %></title>
</head>

<body onload="document.form1.txtJobNumber.focus();">
...
<table  border="0" bordercolor="blue" align="center" class="MainPageText" width="100%">
    ...
    If billtoid="91" then
    %>
    <form method="post" name="form1" id="form1" action="include/fnlrecap.asp">
    JOB NUMBER: <input type="text" name="txtJobNumber" id="txtJobNumber" value="<%=txtJobNumber %>" />
    <%else %>
    <form method="post" name="form1" id="form2" action="orderentry/FleetXFreightOrderConfirmation.asp?x=1&bid=<%=BillToID %>&pid=view">
    JOB NUMBER: <input type="text" name="JID" id="Text1" value="<%=txtJobNumber %>" />
    <%End if %>
    
    <input type="submit" value="Find Job" id="gobutton" />
    </form>
    </td></tr>
