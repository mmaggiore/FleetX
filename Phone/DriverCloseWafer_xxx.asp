    ...
    If PageStatus="CLS" then
			...
			For q=1 to LMNOP
				...
				If trim(FormBarCode(q))>""  then
					...
					SQL = SQL&" WHERE (Fl_dr_ID='"&VehicleID&"') AND (rf_ref='"&FormBarCode(q)&"')"
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                ...
                'Response.Write "Got here #1<br>"
					...
					If oRs.EOF then
						if (trim(Hub)<>"y" and trim(Hub3)<>"y") AND (trim(tempdestination)<>trim(finaldestination) or finaldestination=NULL) then
		...
		<%
			...
			Set RSEVENTS2 = Server.CreateObject("ADODB.Recordset")
				...
				l_csql = L_csql& " AND (fh_id='"&JobNumber&"') AND (fl_leg_status='c') "
						If PageStatus="ONB" then
							l_csql = L_csql& "AND (((fh_status='ACC') OR (fh_status='AC2')) AND ((ref_status is NULL) OR (ref_status <> 'o')))"
