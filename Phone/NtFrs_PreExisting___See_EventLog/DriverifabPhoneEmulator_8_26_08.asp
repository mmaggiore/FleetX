...
'-------------------STARTS THE DROP OFF				
				...
				If HUB3="y" then
					''''Response.Write "got here 1<br>"
					SQL = SQL&" (((Fl_st_ID='CPGP') or (Fl_sf_ID='55') or (FL_ST_ID='TOPPAN') or (FL_SF_ID='TOPPAN')) AND (Fh_Status<>'DPV') )"
				End if								
				For mm=1 to m
				'''''Response.Write "got here 2<br>"
						...
						Select Case JobStatus
							Case "Acknowledged","ONB"
								''''If show2=0 then
									...
									''''show2=show2+1	
									If Request.Cookies("Phone")("sBT_ID")<>"26" AND Request.Cookies("Phone")("sBT_ID")<>"48" AND Request.Cookies("Phone")("sBT_ID")<>"75"  AND MaterialType<>"xxxSecure Waf" then
...
'-------------------STARTS THE PICK UP	
				...
				If HUB3="y" and (trim(vehicleID)<>"611" and trim(vehicleID)<>"612") then
					SQL = SQL&" OR (Fl_sf_ID='CPGP') OR (Fl_sf_ID='55') OR (Fl_sf_ID='72') OR (FL_sf_ID='TOPPAN') OR (FL_st_ID='TOPPAN')"
				End if	
				If HUB3="y" and (trim(vehicleID)="611" or trim(vehicleID)="612") then
					SQL = SQL&" OR (Fl_st_ID='CPGP') OR (Fl_st_ID='55') OR (Fl_st_ID='72') OR (FL_st_ID='TOPPAN')"
				End if	
					...
					If JobNumber<>TempJobNumber then
						If X>0 then
							...
							if trim(fh_bt_id)<>"26" then
								...
								Set Recordset1 = Server.CreateObject("ADODB.Recordset")
								Recordset1.ActiveConnection = DATABASE
								Recordset1.Source = "SELECT count(rf_ref) as NumberOfLots FROM FCREFS WHERE (rf_fh_id='"&TempJobNumber&"') AND (ref_status<>'X')"
								Recordset1.CursorType = 0
								Recordset1.CursorLocation = 2
								Recordset1.LockType = 1
								Recordset1.Open()
						...
						Select Case JobStatus
							Case "Acknowledged","ONB", "ARV", "AC2"
								''''If show=0 then
									...
									'Response.Write "BillToID=***"&BillToID&"****<BR>"	
									'Response.Write "cookie?(sBT_ID)=***"&cookie?("sBT_ID")&"****<BR>"	
									If Request.Cookies("Phone")("sBT_ID")<>"26" AND Request.Cookies("Phone")("sBT_ID")<>"48" AND Request.Cookies("Phone")("sBT_ID")<>"75" then
										%>
