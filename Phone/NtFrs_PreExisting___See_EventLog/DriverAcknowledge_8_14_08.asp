...
''''''''''''''''''''ERROR HANDLING TO PREVENT TIME FLIPPING'''''''''''''''''''''''''''''''
				...
				SQL = SQL&" WHERE (Fl_dr_ID='"&VehicleID&"') AND (Fh_ID='"&txtJobNumber&"')"
...
'------------------------------ACKNOWLEDGES ALL
			If AcknowledgeIt="all" then
				...
				oRs.Open SQL, DATABASE, 1, 3
						...
						If priority="P0" or priority="P1" or MaterialType="Secure Waf" then
							DontRedirect="y"
						End if
						If AcknowledgeIt="all" and Priority<>"P0" and Priority<>"P1" and MaterialType<>"Secure Waf" then
	...
	'					Do while not oRs.eof
						...
						End Select
							...
							If Trim(ToLocation)="55" or Trim(ToLocation)="CPGP" or Trim(ToLocation)="72" or Trim(ToLocation)="TOPPAN" or Trim(ToLocation)="PHO" then
								...
								If (trim(ToLocation)="TOPPAN" or trim(ToLocation)="CPGP" or trim(ToLocation)="PHO") and lcase(JobStatus)="open" and (trim(VehicleID)<>"611" and trim(VehicleID)<>"612" and trim(VehicleID)<>"613") then
							...
							If Trim(FromLocation)="55" or Trim(FromLocation)="72" or Trim(FromLocation)="TOPPAN" or Trim(FromLocation)="PHO" then
								DisplayFromLocation="SB-HUB"
								If trim(VehicleID)="611" or trim(VehicleID)="612" or trim(VehicleID)="613" then
									'response.Write "GOT HERE!!!<BR>"
									DisplayFromLocation=FromLocation
