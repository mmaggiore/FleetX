...
<body onload="doLoad()">
	...
	<table cellspacing="0" cellpadding="0" border="0" width="300">
		...
		'response.Write "vehicleid="&vehicleID&"<BR>"
		'''''''''''''DISPLAYS OTHER VEHICLES FOR RFABSVAN 
		''''''''''''''DISABLED WHILE TESTING IF "RFAB STOCKROOM VAN CAN HANDLE BOTH TYPES OF JOBS!!!!
		'''''If (trim(vehicleID)="113" OR trim(vehicleID)="314") AND XYZ=666 then
		If trim(vehicleID)="123"  then
			''''''''''''FIND P0'S'''''''''''''''''''''''''''''''''''''''''''''''''''
			If DontHide="yes" then    
			    Set oRs = Server.CreateObject("ADODB.Recordset")
			    oRs.CursorLocation = 3
			    oRs.CursorType = 3
			    oRs.ActiveConnection = DATABASE	
			    'Response.Write "vehicleID="&vehicleID&"<BR>"
			    SQL = "SELECT OrdersInVehicle from Mark_Wafer4_P0"
			    oRs.Open SQL, DATABASE, 1, 3
				    If not oRs.eof then
			...
			oRs.ActiveConnection = DATABASE	
			'Response.Write "vehicleID="&vehicleID&"<BR>"
			SQL = "SELECT OrdersInVehicle from Mark_SRVRFAB_P0"
			oRs.Open SQL, DATABASE, 1, 3
				If not oRs.eof then
					OrdersInVehicle = oRs("OrdersInVehicle")
					If OrdersInVehicle>0 then
			...
			oRs.ActiveConnection = DATABASE	
			'Response.Write "vehicleID="&vehicleID&"<BR>"
			SQL = "SELECT OrdersInVehicle from Mark_SRVRFAB_Late"
			oRs.Open SQL, DATABASE, 1, 3
				If not oRs.eof then
					OrdersInVehicle = oRs("OrdersInVehicle")
					If OrdersInVehicle>0 then
						AlertMessage=AlertMessage&"RFAB Stockroom Van has a late order."
					End if
				End if
			oRs.Close
			Set oRs=Nothing
			If DontHide="yes" then
			    ...
			    oRs.ActiveConnection = DATABASE	
			    'Response.Write "vehicleID="&vehicleID&"<BR>"
			    SQL = "SELECT OrdersInVehicle from Mark_Wafer4_Late"
			    oRs.Open SQL, DATABASE, 1, 3
				    If not oRs.eof then
					    OrdersInVehicle = oRs("OrdersInVehicle")
					    If OrdersInVehicle>0 then
						    AlertMessage=AlertMessage&"Wafer 4 has a late order."
					    End if
				    End if
			    oRs.Close
			    Set oRs=Nothing
			...
			oRs.ActiveConnection = DATABASE	
			'Response.Write "vehicleID="&vehicleID&"<BR>"
			SQL = "SELECT OrdersInVehicle from Mark_SRVRFAB_Orders"
			oRs.Open SQL, DATABASE, 1, 3
				If not oRs.eof then
					OrdersInVehicle = oRs("OrdersInVehicle")
					If SRVRFABP0="yes" then
						Response.Write "<font color='red'>"
						else
						Response.Write "<font color='blue'>"
					End if
					'''''Response.Write "SRV-RFAB:"&OrdersInVehicle&"</font>&nbsp;/&nbsp;"
					Response.Write "SRV-RFAB:"&OrdersInVehicle&"</font>&nbsp;"
					'If OrdersInVehicle>0 then
			...
			If DontHide="yes" then
			    ...
			    oRs.ActiveConnection = DATABASE	
			    'Response.Write "vehicleID="&vehicleID&"<BR>"
			    SQL = "SELECT OrdersInVehicle from Mark_Wafer4_Orders"
			    oRs.Open SQL, DATABASE, 1, 3
				    If not oRs.eof then
					    OrdersInVehicle = oRs("OrdersInVehicle")
					    If Wafer2P0="yes" then
						    ...
						    Response.Write "<font color='blue'>"
					    End if					
					    Response.Write "Wafer4:"&OrdersInVehicle&"</font>"
					    'If OrdersInVehicle>0 then
		...
		'''''''''''''DISPLAYS OTHER VEHICLES FOR WAFERS 1-4
		If trim(vehicleID)="312" or trim(vehicleID)="313" or trim(vehicleID)="212" then
			''''''''''''FIND P0'S'''''''''''''''''''''''''''''''''''''''''''''''''''
			Set oRs = Server.CreateObject("ADODB.Recordset")
			oRs.CursorLocation = 3
	...
	'''''''''''''FOR LUNCH/BREAK CODE...IF THEY LEAVE THE PAGE TO CIRCUMVENT THE BREAK PAGE''''''''''
		...
		<tr><td>&nbsp;</td></tr>
		<tr><td>&nbsp;</td></tr>
		<form method="post" action="defaultc.asp" ID="Form16">
			<tr><td><input type="submit" value="do NOT use"></td></tr>
		</form>		
	</table>
