...
<BODY leftMargin=0 TOPMARGIN="0" MARGINWIDTH="0" MARGINHEIGHT="0">
	<TABLE WIDTH="300" border="0" bordercolor="red" cellpadding="0" cellspacing="5" align="left" ID="Table1">
		...
		If Submit>"" then
			...
			Set RSEVENTS2 = Server.CreateObject("ADODB.Recordset")
                ...
                l_csql = l_csql&" fclegs.fl_st_city, fclegs.fl_st_state, fclegs.fl_st_zip, fclegs.fl_st_country "
				...
				End if				
				l_csql = l_csql&" WHERE (fh_bt_id>'') AND ((ref_status<>'X') OR (ref_status is NULL)) "
	...
	<%end if%>		
			...
			'Response.Write "ColspanNumber="&ColspanNumber&"<BR>"
					...
					If (Trim(fl_st_id)="55" AND (Trim(VehicleID)<>"611" AND trim(VehicleID)<>"612")) or Trim(fl_st_id)="72" then
					...
					'End if					
					If ((Trim(fl_sf_id)="55" OR Trim(fl_sf_id)="TOPPAN") AND (Trim(VehicleID)="611" or trim(VehicleID)="612")) then 
						DisplayToLocation="SB-HUB" 
					...
					If Trim(fl_sf_id)="72" then
						DisplayFromLocation="SB-HUB"
							If Fh_Priority="P0" then
								...
								fl_st_rta=DateAdd("n", 90, Fl_firstdrop)
							End if						
					End if			
