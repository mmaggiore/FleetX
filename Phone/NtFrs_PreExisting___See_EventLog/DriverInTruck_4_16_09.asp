		...
		For intRecord = 1 to oRS.PageSize	
					...
					'Response.Write TempX&"</font></td></tr>"
						...
						''''''''''''''''''''''''''''''''
											...
											End if	
											If ((Trim(FromLocation)="55" OR Trim(FromLocation)="TOPPAN" OR Trim(FromLocation)="PHO") AND (Trim(VehicleID)="611" or trim(VehicleID)="612" or trim(VehicleID)="613" or trim(VehicleID)="112" or trim(VehicleID)="123")) then 
												DisplayToLocation="SB-HUB" 
...
''''''''''''''''''''''''''''''''
					...
					If ((Trim(ToLocation)="CPGP" or Trim(FromLocation)="72" or Trim(ToLocation)="TOPPAN" or Trim(ToLocation)="PHO") AND (Trim(VehicleID)="611" OR trim(VehicleID)="612" OR trim(VehicleID)="613" OR trim(VehicleID)="112" OR trim(VehicleID)="123")) then
						DisplayFromLocation="SB-HUB"
					End if	
					If ((Trim(FROMLocation)="CPGP" or Trim(FromLocation)="72" or Trim(FROMLocation)="TOPPAN" or Trim(FROMLocation)="PHO") AND (Trim(VehicleID)="611" OR trim(VehicleID)="612" OR trim(VehicleID)="613" OR trim(VehicleID)="112" OR trim(VehicleID)="123")) then
						DisplayToLocation="SB-HUB"
					End if	
					''''''''''''''''''''''''''''''''''
						'Response.Write "vehicleID="&vehicleID&"<BR>"
