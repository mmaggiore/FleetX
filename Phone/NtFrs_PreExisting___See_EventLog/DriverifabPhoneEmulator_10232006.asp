...
<HTML>
	<HEAD>
		...
		End if
		If AliasCode>"" then Session("AliasCode")=AliasCode end if
		If aliasCode="" then aliasCode=Session("AliasCode") end if
		...
		End if
		If FakeSubmit>"" then
			Session("FakeSubmit")=FakeSubmit
		End if
		If FakeSubmit="" then
			FakeSubmit=Session("FakeSubmit")
		End if
		...
		If PageStatus>"" then 
			...
			'Response.write "sets the session<br>"		
		end if
		If PageStatus="" then 
			PageStatus=Session("PageStatus") 
			'Response.write "takes it from the session<br>"
		end if
...
'-------------------STARTS THE DROP OFF				
				...
				SQL = SQL&" WHERE (fh_ship_dt>'"&now()-30&"') AND (Fl_dr_ID='"&VehicleID&"') and ((Fl_st_ID='"&LocationCode&"')"
				If HUB="y" then
				...
				%>
					<table width="300" cellpadding="0" cellspacing="0" border="0" bordercolor="green" align="left" ID="Table1">
						...
						<tr>
							<td align="center" class="purpleseparator" colspan="3"><b>DROP OFFS AT <%=uCase(LocationCode)%></b></td>
						</tr>
				<%
				If not oRs.EOF then
					%>
						...
						Select Case JobStatus
							Case "Acknowledged","ONB"
								''''If show2=0 then
									...
									'Response.Write "Session(sBT_ID)="&Session("sBT_ID")&"<BR>"
									''''show2=show2+1	
									If Session("sBT_ID")<>"26" AND Session("sBT_ID")<>"48" then
										%>
...
'-------------------STARTS THE PICK UP	
				...
				SQL = "SELECT Fl_SF_ID, Fh_ID, Fl_ST_ID, FH_Status, Fh_Priority FROM fcfgthd INNER JOIN fclegs ON fcfgthd.fh_id = fclegs.fl_fh_id"
				SQL = SQL&" WHERE (fh_ship_dt>'"&now()-30&"') AND  (Fl_dr_ID='"&VehicleID&"') and ((Fl_sf_ID='"&LocationCode&"')"
				If HUB="y" then
				...
				%>
					<!--table width="700" cellpadding="0" cellspacing="0" border="0" align="center" ID="Table2"-->
						<tr>
							<td align="center" class="purpleseparator" colspan="3"><b>PICK UPS AT <%=uCase(LocationCode)%></b></td>
						</tr>
				<%
				If not oRs.EOF then
					%>
