...
<HTML>
	...
	<BODY leftMargin=0 TOPMARGIN="0" MARGINWIDTH="0" MARGINHEIGHT="0">
		<table width="300" cellpadding="0" cellspacing="0" border="0" align="left" ID="Table2">
			...
			SQL = SQL&" WHERE (Fl_dr_ID='"&trim(VehicleID)&"') AND ((FL_Leg_Status='c') OR (FL_Leg_Status is NULL)) AND ((fh_user5='Secure Waf') OR (fh_user5='ITAR') OR (fh_user5='secret') OR (Fh_Priority='P0'))  AND (fh_ship_dt>'"&now()-30&"')"
			...
			Do while not oRs.eof
				...
				If TempToLocation<>ToLocation then
					...
					Recordset1.Source = "SELECT count(fh_id) as NumberOfJobs FROM fcfgthd INNER JOIN fclegs ON fcfgthd.fh_id = fclegs.fl_fh_id WHERE ((Fh_Priority='P0') or (fh_user5='Secure Waf') or (fh_user5='ITAR') or (fh_user5='secret')) AND ((FL_Leg_Status='c') OR (FL_Leg_Status is NULL)) AND (Fl_dr_ID='"&VehicleID&"') AND (fh_ship_dt>'"&now()-30&"') AND "
					'''''If VehicleID=124 then
...
'''''''''''''''''''''''''''''''''''''''					
					...
					If (Trim(FromLocation)="55" or Trim(FromLocation)="72" or Trim(FromLocation)="TOPPAN" or Trim(FromLocation)="PHO")  and (trim(VehicleID)<>"611" AND trim(VehicleID)<>"612" AND trim(VehicleID)<>"613" AND trim(VehicleID)<>"112" AND trim(VehicleID)<>"123") then
						...
						'Response.Write "got here 3<BR>"
					End if	
					If trim(VehicleID)="613" and (trim(ToLocation)="TISHERMA" or trim(ToLocation)="TISHERMANRET") AND trim(FromLocation)<>"PHO" AND trim(FromLocation)<>"CSSF" then
						'REsponse.Write "GOT HERE!<BR>"
						DisplayFromLocation="SB-HUB"
					End if
					Select Case DisplayToLocation
