...
'-------------------STARTS THE DROP OFF	
				...
				SQL = "SELECT Fl_SF_ID, Fh_ID, fh_User5, Fl_ST_ID, fl_st_rta, fl_firstdrop, fl_sf_comment, fh_bt_id, FH_Status, Fh_Priority FROM fcfgthd INNER JOIN fclegs ON fcfgthd.fh_id = fclegs.fl_fh_id"
				SQL = SQL&" WHERE (Fl_dr_ID='"&VehicleID&"') AND ((FL_Leg_Status='c') OR (FL_Leg_Status is NULL)) AND (fh_ship_dt>'"&now()-30&"')"
		...
		For intRecord = 1 to oRS.PageSize	
					...
					'Response.Write TempX&"</font></td></tr>"
						...
						''''''''''''''''''''''''''''''''
											...
											If (Trim(ToLocation)="TOPPAN" or Trim(ToLocation)="CPGP" or Trim(ToLocation)="72" or Trim(ToLocation)="PHO" or Trim(ToLocation)="TISHERMANRET") and (trim(VehicleID)<>"611" AND trim(VehicleID)<>"612" AND trim(VehicleID)<>"613" AND trim(VehicleID)<>"112" AND trim(VehicleID)<>"123") then
												DisplayToLocation="SB-HUB"
												'response.write "Got here 1<BR>"
											End if
											If (Trim(FromLocation)="55" and (VehicleID<>"611" OR VehicleID<>"612" OR VehicleID<>"613" OR VehicleID<>"112" OR VehicleID<>"123")) or Trim(FromLocation)="72" then
												DisplayFromLocation="SB-HUB"
												'response.write "Got here 2<BR>"
											End if	
											If ((Trim(FromLocation)="55" OR Trim(FromLocation)="TOPPAN" OR Trim(FromLocation)="PHO" OR Trim(FromLocation)="HFABRET") AND (Trim(VehicleID)="611" or trim(VehicleID)="612" or trim(VehicleID)="613" or trim(VehicleID)="112" or trim(VehicleID)="123")) then 
												DisplayToLocation="SB-HUB" 
												ToName="South Building HUB"
												'response.write "Got here 3<BR>"
											End if
											If trim(VehicleID)="613" and trim(ToLocation)="TISHERMA" or Trim(ToLocation)="TISHERMANRET" then
												'REsponse.Write "GOT HERE!<BR>"
												DisplayFromLocation="SB-HUB"
											End if											
										''''''''''''''''''''''''''''''''''
...
'-------------------STARTS THE PICK UP	
				...
				SQL = "SELECT Fl_SF_ID, Fh_ID, fh_User5, fh_bt_id, Fl_ST_ID, fl_st_rta, fl_firstdrop, fl_sf_comment, FH_Status, Fh_Priority, fh_user5 FROM fcfgthd INNER JOIN fclegs ON fcfgthd.fh_id = fclegs.fl_fh_id"
				SQL = SQL&" WHERE (Fl_dr_ID='"&VehicleID&"') AND ((FL_Leg_Status='c') OR (FL_Leg_Status is NULL)) AND (fh_ship_dt>'"&now()-30&"')"
...
''''''''''''''''''''''''''''''''
					...
					If (Trim(ToLocation)="CPGP" or  Trim(ToLocation)="TOPPAN" or  Trim(ToLocation)="PHO") and (Trim(VehicleID)<>"611" AND trim(VehicleID)<>"612" AND trim(VehicleID)<>"613" AND trim(VehicleID)<>"112" AND trim(VehicleID)<>"123") then
						DisplayToLocation="SB-HUB"
					...
					If ((Trim(FromLocation)="55" or Trim(FromLocation)="72" or Trim(FromLocation)="TOPPAN" or Trim(FromLocation)="PHO") AND (Trim(VehicleID)<>"611" AND trim(VehicleID)<>"612" AND trim(VehicleID)<>"613" AND trim(VehicleID)<>"112" AND trim(VehicleID)<>"123")) then
						DisplayFromLocation="SB-HUB"
						'response.write "Got here 2xxx<BR>"
					End if	
					If ((Trim(ToLocation)="CPGP" or Trim(FromLocation)="72" or Trim(ToLocation)="TOPPAN" or Trim(ToLocation)="PHO") AND (Trim(VehicleID)="611" OR trim(VehicleID)="612" OR trim(VehicleID)="613" OR trim(VehicleID)="112" OR trim(VehicleID)="123")) then
						DisplayFromLocation="SB-HUB"
						'response.write "Got here 3xxx<BR>"
					End if	
					If ((Trim(FROMLocation)="CPGP" or Trim(FromLocation)="72" or Trim(FROMLocation)="TOPPAN" or Trim(FROMLocation)="PHO" or Trim(FromLocation)="HFABRET") AND (Trim(VehicleID)="611" OR trim(VehicleID)="612" OR trim(VehicleID)="613" OR trim(VehicleID)="112" OR trim(VehicleID)="123")) then
								If (Trim(VehicleID)<>"613" and Trim(FromLocation)<>"HFABRET") AND (Trim(VehicleID)<>"612" and Trim(ToLocation)<>"HFABRET") then
									DisplayToLocation="SB-HUB"
									'response.write "Got here 4xxx<BR>"
									'response.write "VehicleID="&VehicleID&"<BR>"
								End if
						if Trim(FromLocation)="HFABRET" and trim(VehicleID)="612" then
							'response.write "Got here 5xxx<BR>"
							DisplayFromLocation=FromLocation
						End if
						'Response.Write "Got here 1<BR>"
					End if	
