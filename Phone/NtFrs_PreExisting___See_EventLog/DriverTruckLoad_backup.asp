		...
		<table width="300" cellpadding="0" cellspacing="0" border="0" align="left" ID="Table2">
			...
			Do while not oRs.eof
				...
				If TempToLocation<>ToLocation then
					...
					If Trim(ToLocation)="55" or Trim(ToLocation)="CPGP" then
						DisplayToLocation="SB-HUB"
					End if
					If Trim(FromLocation)="55" or Trim(FromLocation)="CPGP" then
					...
					Recordset1.Source = "SELECT count(fh_id) as NumberOfJobs FROM fcfgthd INNER JOIN fclegs ON fcfgthd.fh_id = fclegs.fl_fh_id WHERE (Fl_dr_ID='"&VehicleID&"') AND (fh_ship_dt>'"&now()-30&"') AND "
					'''''If VehicleID=124 then
			...
			oRs.CursorType = 3
			oRs.ActiveConnection = DATABASE	
			SQL = "SELECT distinct(Fl_SF_ID), Fh_ID, Fl_ST_ID, fl_st_rta, fl_firstdrop, fh_bt_id, FH_Status, Fh_Priority, fh_user5 FROM fcfgthd INNER JOIN fclegs ON fcfgthd.fh_id = fclegs.fl_fh_id"
			SQL = SQL&" WHERE (Fl_dr_ID='"&trim(VehicleID)&"') AND (fh_ship_dt>'"&now()-30&"')"
			If BillToID="48" then
			...
			Do while not oRs.eof
				...
				If TempToLocation<>ToLocation OR TempFromLocation<>FromLocation then
					...
					Set Recordset1 = Server.CreateObject("ADODB.Recordset")
					Recordset1.ActiveConnection = DATABASE
					Recordset1.Source = "SELECT count(fh_id) as NumberOfJobs FROM fcfgthd INNER JOIN fclegs ON fcfgthd.fh_id = fclegs.fl_fh_id WHERE (Fl_dr_ID='"&VehicleID&"') AND (fh_ship_dt>'"&now()-30&"') "
					If BillToID="48" then
...
'''''''''''''''''''''''''''''''''''''''					
					Set Recordset1 = Server.CreateObject("ADODB.Recordset")
					Recordset1.ActiveConnection = DATABASE
					Recordset1.Source = "SELECT count(fh_id) as Any300 FROM fcfgthd INNER JOIN fclegs ON fcfgthd.fh_id = fclegs.fl_fh_id WHERE (Fl_dr_ID='"&VehicleID&"') AND (fh_user5='300 mm Waf') AND (fh_ship_dt>'"&now()-30&"') "
					If BillToID="48" then
						Recordset1.Source =Recordset1.Source& " AND (fh_status='PUO') "
						else
						Recordset1.Source =Recordset1.Source& " AND ((fh_status='ACC') "
