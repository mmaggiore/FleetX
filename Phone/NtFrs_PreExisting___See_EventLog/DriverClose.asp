				...
				If trim(FormBarCode(q))>""  then
					...
					If PageStatus="ONB" then
						If BillToID="48" or trim(vehicleID)="198" then
							...
							If (trim(VehicleID)="124" or trim(VehicleID)="123" or trim(VehicleID)="113") and LocationCode<>"ESTK" then
							...
							End if								
						End if
						If (trim(VehicleID)<>"124" AND VehicleID<>"123" AND VehicleID<>"113") or LocationCode="ESTK" then
							SQL = SQL& "AND ((fl_sf_id='"&LocationCode&"') "
						End if
						''''''''''''THIS MIGHT NEED TO GO AWAY!!!!
						'response.Write "GOT HERE!<BR>xxx"
						'Response.Write "hub="&hub&"XXX<BR>"
						If trim(HUB)="y" then
							...
							sql = sql&" OR (Fl_sf_ID='TOPPAN')"
							sql = sql&" OR (Fl_sf_ID='DFAB1')"
							sql = sql&" OR (Fl_sf_ID='DFABR')"
							sql = sql&" OR (Fl_sf_ID='PHO')"
							sql = sql&" OR (Fl_sf_ID='DM5SR')"
						End if	
						If (VehicleID<>"124" AND VehicleID<>"123" AND VehicleID<>"113") or LocationCode="ESTK" then	
							SQL = SQL& " ) "						
						End if
					end if					
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''					
			...
			''''''''''''''''''''email notification BEGIN
						...
						''''''''''''''''''''''''''''''''''''''''''''''''
							...
							If ((trim(VehicleID)<>"124" AND trim(VehicleID)<>"113" ) or trim(LocationCode)="ESTK") and trim(LocationCode)<>"13536" or trim(The2Address)="TISHERMA"  then
			...
			For q=1 to 12
				If trim(FormBarCode(q))>""  then
					...
					If PageStatus="CLS" then
						...
						End if													
					End if
					'Response.Write "XXXYYYZZZSQL="&SQL&"<BR>"
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''					
					oRs.Open SQL, DATABASE, 1, 3
					If oRs.EOF then
						ErrorMessage=ErrorMessage&" "&FormBarCode(q)&"  is not accepted.<br>Check Paper Work/Call Supervisor<br>"
					...
					If not oRs.eof then
						...
						If BillToID="48" or trim(vehicleID)="198" or BillToID="75" or BillToID="36" or  BillToID="80" then
						...
						If oRs.EOF then
							If (Hub="y" or CloseThis="n") then
								...
								If BillToID="48" AND len(trim(jobcomment))>1 then
									...
									objMail.To = Email
									objMail.cc = "mark.maggiore@logisticorp.us"
									objMail.Subject = "Job #"& TheJobNumber &" Comment"
									objMail.MailFormat = cdoMailFormatMIME
									objMail.BodyFormat = cdoBodyFormatHTML
									objMail.Body = Body
									objMail.Send
								...
								If trim(vehicleID)="199xxx" then
									...
									l_cSQL = "UPDATE FCLEGS SET fl_un_id='"& Nextfl_un_id &"', fl_dr_id='"& Nextfl_dr_id &"', fl_Leg_Status = 'c' "
								...
								If BillToID="48" or trim(vehicleID)="198" then
									...
									Select Case NextToAddress
										...
										Case "6430" '''''Sherman final destination
											Nextfl_un_id="6"
											Nextfl_dr_id="6"
										Case "6550" '''''Spring Creek final destination
											Nextfl_un_id="3"
											Nextfl_dr_id="3"
										Case "12500" '''''Forest Lane final destination
											Nextfl_un_id="7"
											Nextfl_dr_id="7"
										Case "13536F" '''''12536 final destination
											Nextfl_un_id="7"
											Nextfl_dr_id="7"										
										Case Else
											Nextfl_un_id="8"
								...
								''''''''FIND NOTIFICATION INFORMATION!!!!!
									...
									If not Recordset1.eof then
										...
										"drop zone "& LocationCode &" at "& now() & "<BR><BR>REPORT PROBLEMS TO: dsb_mrp@list.ti.com<br><br>"
										'Recipient=FirstName&" "&LastName
										'Email="KWETI.Mailbox@am.kwe.com"
										'Email="mark@maggiore.net"
										Set objMail = CreateObject("CDONTS.Newmail")
										objMail.From = "system.notification@logisticorp.us"
										objMail.To = MailEmailAddress
										...
										objMail.Subject = "Electronic Sales Order #"& mailref
										objMail.MailFormat = cdoMailFormatMIME
										objMail.BodyFormat = cdoBodyFormatHTML
										objMail.Body = Body
										objMail.Send
	...
	<TABLE WIDTH="300" border="0" bordercolor="red" cellpadding="0" cellspacing="5" align="left" ID="Table1">
		...
		<%
			...
			If JobNumber>"" then optJobSel="ByJob" end if
				...
				'l_csql = l_csql&" WHERE (ref_status<>'c') AND (Fl_dr_ID='"&VehicleID&"') AND ((FL_Leg_Status='c') OR (FL_Leg_Status is NULL)) AND fh_ship_dt>'"&now()-30&"'"
				l_csql = l_csql&" WHERE (Fl_dr_ID='"&VehicleID&"') AND ((FL_Leg_Status='c') OR (FL_Leg_Status is NULL)) AND fh_ship_dt>'"&now()-30&"'"
						If PageStatus="ONB" then
							If BillToID="48" or trim(vehicleID)="198" then
								...
								If trim(VehicleID)="124" or trim(VehicleID)="123" or trim(VehicleID)="113" then
								...
								End if								
							End if
							If VehicleID<>"124" and vehicleID<>"123" and VehicleID<>"113" then
							...
							If HUB="y" then
								...
								l_csql = l_csql&" OR (Fl_sf_ID='TOPPAN')"
								l_csql = l_csql&" OR (Fl_sf_ID='DFABR')"
								l_csql = l_csql&" OR (Fl_sf_ID='DFAB1')"
								l_csql = l_csql&" OR (Fl_sf_ID='PHO')"
								l_csql = l_csql&" OR (Fl_sf_ID='DM5SR')"
...
<%
				...
				''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''		
						If PageStatus="CLS" and ((BillToID="48" and NeedPOD="y") or BillToID="75" or BillToID="36" or BillToID="80") then
			...
			<tr>
				<td colspan="2">
					<%
					'response.Write "billtoid="&billtoid&"<BR>"
					if Pagestatus="CLS" and (BillToID="48" or BillToID="75" or BillToID="36" or BillToID="80" ) then%>
