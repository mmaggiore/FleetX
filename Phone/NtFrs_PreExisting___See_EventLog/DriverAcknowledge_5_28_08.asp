...
'------------------------------ACKNOWLEDGES ALL
			If AcknowledgeIt="all" then
				...
				SQL = SQL&" AND (((Fh_Status='OPN') AND (fl_sf_ID<>'55'))"
				...
				oRs.Open SQL, DATABASE, 1, 3
						...
						If priority="P0" or priority="P1" then
							DontRedirect="y"
						End if
						If AcknowledgeIt="all" and Priority<>"P0" and Priority<>"P1" then
							If FORMJOBSTATUS<>"ARV" then
								Set oConn = Server.CreateObject("ADODB.Connection")
								oConn.ConnectionTimeout = 100
								oConn.Provider = "MSDASQL"
...
<%
				...
				oRs.CursorType = 3
				oRs.ActiveConnection = DATABASE	
				SQL = "SELECT Fl_SF_ID, Fh_ID, fh_ship_dt, fh_bt_id, fh_user5, Fl_ST_ID, Fl_St_Rta, Fl_FirstDrop, FH_Status, Fh_Priority FROM fcfgthd INNER JOIN fclegs ON fcfgthd.fh_id = fclegs.fl_fh_id"
				SQL = SQL&" WHERE (Fl_dr_ID='"&VehicleID&"')"
				''''If VehicleID=124 then
					SQL = SQL&" AND ((((Fh_Status='ARV') AND (fl_st_id<>'TOPPAN')) AND (Fl_SecAcc is NULL)) "
					'''''else
					SQL = SQL&" OR ((Fh_Status='OPN') AND (fl_sf_id<>'55'))) "
				'''''End if
...
If CInt(intPage) > CInt(intPageCount) Then intPage = intPageCount
	...
	'					Do while not oRs.eof
						...
						End Select
							...
							If Trim(ToLocation)="55" or Trim(ToLocation)="72" or Trim(ToLocation)="TOPAN" then
								DisplayToLocation="SB-HUB"
							End if
							If Trim(FromLocation)="55" or Trim(FromLocation)="72" or Trim(FromLocation)="TOPAN" then
