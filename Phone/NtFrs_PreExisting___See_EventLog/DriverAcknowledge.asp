...
''''''''''''''''''''ERROR HANDLING TO PREVENT TIME FLIPPING'''''''''''''''''''''''''''''''
				...
				SQL = SQL&" WHERE (Fl_dr_ID='"&trim(VehicleID)&"') AND (Fh_ID='"&trim(txtJobNumber)&"') AND ((FL_Leg_Status='c') OR (FL_Leg_Status is NULL))"
...
''''''''''''''''''''''FINDS EMAIL LIST
						If MaterialType = "ITAR" then
							...
							objMail.To = "itar@logisticorp.us"
							
							objMail.cc= "mark.maggiore@logisticorp.us"

							objMail.Subject = "ITAR Pick Up Notification"
...
'------------------------------ACKNOWLEDGES ALL
			If AcknowledgeIt="all" then
				...
				SQL = "SELECT Fl_SF_ID, Fh_ID, fl_sf_comment, fh_ship_dt, Fl_ST_ID, FH_Status, Fh_Priority, Fh_User5 FROM fcfgthd INNER JOIN fclegs ON fcfgthd.fh_id = fclegs.fl_fh_id"
				...
				oRs.Open SQL, DATABASE, 1, 3
						...
						If priority="P0" or priority="P1" or MaterialType="ITAR" or MaterialType="Secure Waf" or MaterialType="secret" then
							DontRedirect="y"
						End if
						If AcknowledgeIt="all" and Priority<>"P0" and Priority<>"P1" and MaterialType<>"Secure Waf" and MaterialType<>"ITAR" and MaterialType<>"secret" then
...
<%
				...
				oRs.CursorType = 3
				oRs.ActiveConnection = DATABASE	
				SQL = "SELECT Fl_SF_ID, Fl_SF_Comment, Fh_ID, Fh_User5, fh_ship_dt, fh_bt_id, fh_user5, Fl_ST_ID, Fl_St_Rta, Fl_FirstDrop, FH_Status, Fh_Priority FROM fcfgthd INNER JOIN fclegs ON fcfgthd.fh_id = fclegs.fl_fh_id"
				SQL = SQL&" WHERE (Fl_dr_ID='"&VehicleID&"') AND ((FL_Leg_Status='c') OR (FL_Leg_Status is NULL))"
				''''If VehicleID=124 then
					'''''SQL = SQL&" AND ((((Fh_Status='ARV') AND (fl_st_id<>'TOPPAN')) AND (Fl_SecAcc is NULL)) "
...
If CInt(intPage) > CInt(intPageCount) Then intPage = intPageCount
	...
	'					Do while not oRs.eof
						...
						End Select
							...
							End if							
							If Trim(ToLocation)="55" or Trim(ToLocation)="CPGP" or Trim(ToLocation)="72" or Trim(ToLocation)="TOPPAN" or (Trim(ToLocation)="PHO" AND Trim(FromLocation)<>"TISHERMANRET") or (Trim(ToLocation)="TISHERMANRET" AND Trim(FromLocation)<>"PHO") then
								...
								'Response.Write "FromLocation="&FromLocation&"***<BR>"
								'Response.Write "JobStatus="&JobStatus&"***<BR>"	
								'Response.Write "TempJobStatus="&TempJobStatus&"***<BR>"								
								If ((trim(ToLocation)="TOPPAN" or trim(ToLocation)="CPGP" or (trim(ToLocation)="PHO" AND trim(FromLocation)<>"TISHERMANRET"))AND TempJobStatus<>"ARV") or Trim(ToLocation)="TISHERMANRET" and lcase(JobStatus)="open" and (trim(VehicleID)<>"611" and trim(VehicleID)<>"612" and trim(VehicleID)<>"613" and trim(VehicleID)<>"112" and trim(VehicleID)<>"123") then
									DisplayFromLocation=trim(FromLocation)
							...
							End if
							'Response.Write "VehicleID="&VehicleID&"***<BR>"
							If trim(ToLocation)="TISHERMANRET" and trim(FromLocation)="TOPPAN" and trim(VehicleID)="611" then
								DisplayFromLocation=Trim(FromLocation)
								DisplayToLocation="SB-HUB"
							End if

							'response.Write "VehicleID="&trim(VehicleID)&"***<br>"
							'response.Write "FromLocation="&trim(FromLocation)&"***<br>"
							'response.Write "ToLocation="&trim(ToLocation)&"***<br>"
							
							If Trim(FromLocation)="55" or Trim(FromLocation)="72" or (Trim(FromLocation)="TOPPAN"  AND Trim(ToLocation)<>"TISHERMANRET") or (Trim(FromLocation)="PHO" AND Trim(ToLocation)<>"TISHERMANRET") or Trim(FromLocation)="HFABRET" then
								...
								If (trim(VehicleID)="611" or trim(VehicleID)="612" or trim(VehicleID)="613" or trim(VehicleID)="112" or trim(VehicleID)="123") AND Trim(ToLocation)<>"PHO" AND Trim(ToLocation)<>"HFABRET" AND (Trim(ToLocation<>"TISHERMANRET") AND Trim(FromLocation<>"PHO")) then
									'response.Write "GOT HERE!!!<BR>"
							...
							If (trim(VehicleID)="123" and (trim(ToLocation)="TISHERMA" OR trim(ToLocation)="TISHERMANRET")) OR (Trim(VehicleID)="613" AND trim(FromLocation)="TISHERMANRET" AND trim(ToLocation)<>"PHO") then
								'REsponse.Write "GOT HERExxx!<BR>"
								DisplayToLocation="SB-HUB"
							End if
							If trim(VehicleID)="613" and ((trim(ToLocation)="TISHERMA" OR trim(ToLocation)="TISHERMANRET") AND (trim(FromLocation)<>"CSSF")) AND (trim(FromLocation)<>"TOPPAN" AND trim(FromLocation)<>"PHO")then
								'REsponse.Write "GOT HERE!<BR>"
								DisplayFromLocation="SB-HUB"
							End if							
