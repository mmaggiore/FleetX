...
<BODY leftMargin=0 TOPMARGIN="0" MARGINWIDTH="0" MARGINHEIGHT="0">
	...
	<%end if%>		
			...
			'Response.Write "ColspanNumber="&ColspanNumber&"<BR>"
					...
					If (((Trim(fl_sf_id)="55" OR Trim(fl_sf_id)="TOPPAN" OR (Trim(fl_sf_id)="PHO")) AND Trim(fl_st_id)<>"TISHERMANRET") AND (Trim(VehicleID)="611" or trim(VehicleID)="612" or trim(VehicleID)="613" or trim(VehicleID)="112" or trim(VehicleID)="123")) then 
						If trim(fl_st_id)<>"HFABRET" then
							DisplayToLocation="SB-HUB" 
					...
					''''''''''''''''''''''''''''''''''
						...
						'Response.Write "fl_st_id="&fl_st_id&"<BR>"
							If trim(VehicleID)="123" and trim(fl_st_id)="TISHERMA" then
								...
								DisplayToLocation="SB-HUB"
							End if
							If trim(VehicleID)="613" and trim(fl_st_id)="TISHERMA" and Trim(Fl_sf_id)<>"CSSF" then
								'REsponse.Write "GOT HERE!<BR>"
								DisplayFromLocation="SB-HUB"
							End if												
					''''''''''''''''''''''''''''''''''								
