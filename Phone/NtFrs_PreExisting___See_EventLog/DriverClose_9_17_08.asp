		...
		If PageStatus="ONB" then
			For q=1 to 12
				If trim(FormBarCode(q))>""  then
					...
					SQL = SQL&" WHERE (Fl_dr_ID='"&VehicleID&"') AND (FL_Leg_Status='c') AND (fh_ship_dt>'"&now()-30&"') AND (rf_ref='"&FormBarCode(q)&"')"
					If PageStatus="ONB" then
						If BillToID="48" then
	...
	<TABLE WIDTH="300" border="0" bordercolor="red" cellpadding="0" cellspacing="5" align="left" ID="Table1">
		...
		<%
			...
			If JobNumber>"" then optJobSel="ByJob" end if
				...
				l_csql = l_csql&" WHERE (Fl_dr_ID='"&VehicleID&"') AND (FL_Leg_Status='c') AND fh_ship_dt>'"&now()-30&"'"
						If PageStatus="ONB" then
							If BillToID="48" then
