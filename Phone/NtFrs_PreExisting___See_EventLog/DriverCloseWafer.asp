...
<% 
'Response.Write "now THIS page<BR>"
Dim FormBarCode(12)
                    ...
                    'Response.Write "Got here #1<br>"
						...
						'response.write "Hub3="&Hub3&"<BR>"
							...
							if trim(Hub)<>"y" and trim(Hub3)<>"y" then
								'response.write "Got Here #1 (ONB no hub)"& TheJobNumber &"<br>"
								...
								'response.write "Got Here #2 (Second onb HUB)"& TheJobNumber &"<br>"
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''					
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
					SQL = "SELECT fcfgthd.fh_id, fclegs.fl_sf_rta, fcfgthd.fh_status, fcfgthd.fh_ship_dt, fcfgthd.fh_carr_id, fclegs.fl_sf_id, fclegs.fl_sf_name, fclegs.fl_st_id, fclegs.fl_st_name, fclegs.fl_t_atp, fclegs.fl_t_atd, fclegs.fl_pod, fcfgthd.fh_custpo, fcfgthd.fh_priority, fcfgthd.fh_user5, fcrefs.rf_ref FROM fcfgthd INNER JOIN fclegs ON fcfgthd.fh_id = fclegs.fl_fh_id INNER JOIN fcrefs ON fclegs.fl_fh_id = fcrefs.rf_fh_id "	
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''					
					...
					If not oRs.eof and trim(ErrorMessage)="" then
						...
						fh_carr_id=oRs("fh_carr_id")
						tempOrigination = trim(oRs("fl_sf_id"))
						tempDestination = trim(oRs("fl_st_id"))
                        fh_user5=trim(oRs("fh_user5"))
    					...
    					'Response.write "PODID=***"&PODID&"***<BR>"					
                        If ucase(fh_user5)="ITAR" AND (trim(PODID)="xxx") then
                        	ErrorMessage="A POD 'signature' is required on all ITARs."
    					End if                         
						'Response.write "TheJobNumber=***"&TheJobNumber&"***<BR>"
						'Response.write "TheBarCode=***"&TheBarCode&"***<BR>"
						...
						If oRs.EOF then
							...
							if Hub<>"y" and Hub3<>"y" then
								...
								oConn.Execute "PHONE_CHANGE_STATUS " & sCriteria 
  								'''''''''''''''''''CSSF PO/P1 FROM 5AM to 7:30PM DELIVERY NOTIFICATIONS''''''''''
  							  	TheHour=hour(now())
  							  	TheMinute=minute(now())
                               ' Response.Write "LocationCode="&LocationCode&"<BR>"
                               ' Response.Write "MailPriority="&MailPriority&"<BR>"
                                'Response.Write "TheHour="&TheHour&"<BR>"
                                'Response.Write "TheMinute="&TheMinute&"<BR>" 
                              							  	
  							  	If Trim(LocationCode="CSSF") and (mailpriority="P0" or mailpriority="P1") AND ((TheHour>=5 and TheHour<=18) OR (TheHour=19 AND TheMinute<=30)) then
										Body = "WAFER(S) #"&WaferList&" has/have just been delivered to "& LocationCode &" from "& tempOrigination &".<br><br>"& _
										...
										objMail.From = "system.monitor@logisticorp.us"
										objMail.To = "mark.maggiore@logisticorp.us"
										objMail.CC = "ldawson@ti.com;m-neil@ti.com;b-julian1@ti.com;tracy.johnson@ti.com;m-haralson@ti.com;eshp@list.ti.com;dfab-ci@list.ti.com"
										objMail.Subject = "CSSF P0/P1 Delivery"
										objMail.MailFormat = cdoMailFormatMIME
										objMail.BodyFormat = cdoBodyFormatHTML
										objMail.Body = Body
										objMail.Send
								...
								SQL_9876="PHONE_CHANGE_STATUS '" & TheJobNumber & "', '53', 'ARV', '', '', '" & UserID & "', '" & UnitID & "' " 
								'Response.Write "SQL_9876="&SQL_9876&"<BR>"
								oConn.Execute SQL_9876
								
								If tempDestination="TISHERMANRET" AND trim(fh_carr_id)="NTCS" then
									SQL_5467="PR_DISPATCH_NTSC '53', 'ARV', '', '" & TheJobNumber & "' " 
									'Response.Write "SQL_5467="&SQL_5467&"<BR>"
									oConn.Execute SQL_5467
									Body = "RE:&nbsp;&nbsp; Job #&nbsp;&nbsp;"& TheJobNumber &"<br><br>"   & _
									"From: SB-HUB (Dallas) to TI-Sherman.<br><br>"   & _
									"Ready to be picked up.<br><br>"  
									'Recipient=FirstName&" "&LastName
									SentToEmail="mark.maggiore@logisticorp.us;northtexascourier@hotmail.com;9032717026@mms.att.net"
									...
									objMail.Subject = "TI-Dallas Pick Up Now Ready"
									objMail.MailFormat = cdoMailFormatMIME
									objMail.BodyFormat = cdoBodyFormatHTML
									objMail.Body = Body
									objMail.Send
						...
						If fh_status<>"DPV" AND (tempDestination="HBUMP" or tempDestination="TOPPAN" or tempDestination="CPGP" or tempDestination="PHO" or tempDestination="TISHERMANRET") then	
							'response.write "GOT HERE!!!<BR>"
						...
						''''''''''''''''''''''''''''''''''''''''	
						''''''''''''''''''CHANGES THE PRIORITY TIME FOR TOPPAN AND HBUMP'''''''''''''''''''''''
						If fh_status<>"DPV" AND (tempOrigination="HBUMP" or tempOrigination="TOPPAN" or tempOrigination="55" or tempOrigination="PHO" or tempOrigination="TISHERMANRET") then	
							''Response.Write "GOT HERE!!!<BR>"
