...
''''''''''''''''''''''''''''''''''''''''''		
		...
		'-----FIND THE JOBS THAT NEED TO BE ACKNOWLEDGED------------------------------------------------------------
			'Response.Write "VehicleID="&VehicleID&"<BR>"
				...
				'Response.Write "vehicleID="&vehicleID&"<BR>"
				SQL = "SELECT fh_priority, fh_user5 FROM VW_Driver_FindOpenJobs"
		...
		''''''''''''NEW VIEW TO SHOW THE OPEN ORDERS/PO's/LATES FOR THE OTHER VEHICLES''''''''''
				...
				oRs.ActiveConnection = DATABASE	
				'Response.Write "vehicleID="&vehicleID&"<BR>"
				SQL = "SELECT fh_priority, fh_user5, fl_un_id, fl_st_rta FROM VW_Driver_FindAllOpenJobsForAllWaferTrucks"
				SQL = SQL&" WHERE (Fl_UN_ID IN (303551,303552,303553, 303554))"
					...
					Do while not oRs.eof 
				       ...
				       ' Response.Write "X66="&X66&"<BR>"
						...
						Select Case AllVehicle
						    ...
						    Case 303554
						        NumJobs303554=NumJobs303554+1
						        If AllPriority="P0" or AllPriority="P1" or AllPriority="XP" or AllMaterialType="Secure Waf" or AllMaterialType="ITAR" then
						            XP303554=XP303554+1
						        End if
						        If AllDueTime>Now() then
						            Late303554=Late303554+1
						        End if	
						        'Response.Write "NumJobs303554="&NumJobs303554&"<BR>"
						        'Response.Write "XP303554="&XP303554&"<BR>"
						        'Response.Write "Late303554="&Late303554&"<BR>"							        					        						        
						End Select
...
<body onload="doLoad()">
	...
	<table cellspacing="0" cellpadding="0" border="0" width="300">
		...
		If NumJobs303554>10 then JobFont303554="red" else JobFont303554="Green" end if 
		...
		If XP303554>0 then XPFont303554="red" else XPFont303554="Green" end if 
		...
		If Late303554>0 then LateFont303554="red" else LateFont303554="Green" end if 		
		%>
		    <tr><td>&nbsp;</td></tr>
		    ...
		    <%If UnitID<>"303554" then %>
		        <tr><td class="mainpagetextbold">W4: <font color="<%=JobFont303554%>">Jobs=<%=NumJobs303554%></font> / <font color="<%=XPFont303554%>">P0=<%=XP303554%></font> / <font color="<%=LateFont303554%>">Late=<%=Late303554%></font></td></tr>
		    <%End if%>
		<%end if %>
		<tr><td>&nbsp;</td></tr>
