...
''''''''''''''''''''email notification END				
		...
		End if
			Set Recordset1 = Server.CreateObject("ADODB.Recordset")
			Recordset1.ActiveConnection = DATABASE
			Recordset1.Source = "SELECT fcfgthd.fh_id, fcfgthd.fh_bt_id, fclegs.fl_sf_id FROM fcfgthd INNER JOIN fcrefs ON fcfgthd.fh_id = fcrefs.rf_fh_id INNER JOIN fclegs ON fcrefs.rf_fh_id = fclegs.fl_fh_id WHERE (fcfgthd.fh_bt_id = '36' OR fcfgthd.fh_bt_id = '38') AND (fcrefs.rf_ref = '"& HAWB &"') AND (fcfgthd.fh_statcode <> '9') AND (fcfgthd.fh_statcode <> '99') AND (fcfgthd.fh_statcode <> '98')"
			Recordset1.CursorType = 0
			Recordset1.CursorLocation = 2
			Recordset1.LockType = 1
			Recordset1.Open()
	...
	<%end if%>
			...
			Select Case Pagestatus
				...
				Case Else			
			%>
			<FORM ACTION="DriverExceptions_IFAB.asp" method="post" name="thisForm" ID="Form6">
					<TR> 
						<td> 
