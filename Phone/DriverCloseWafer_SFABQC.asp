...
IF Submit="submit" THEN
    ...
    If PageStatus="CLS" then
				...
				If trim(FormBarCode(q))>""  then
					...
					If not oRs.eof and trim(ErrorMessage)="" then
                        ...
                        If trim(ErrorMessage)="" then
						    ...
						    End if
                            l_cSQL = l_cSQL&" WHERE  (rf_box = '" &TheBarCode& "') and (ref_status='o')"
		...
		SQL75 = "SELECT distinct rf_fh_id FROM fcrefs INNER JOIN fcfgthd ON fcrefs.rf_fh_id = fcfgthd.fh_id "
		SQL75 = SQL75&" WHERE (rf_box='"&TheBarCode&"')"
		SQL75 = SQL75&" AND (Ref_Status='c') AND (fcfgthd.fh_status <> 'CLS') AND (fcfgthd.fh_status <> 'CAN')"
		oRs75.Open SQL75, DATABASE, 1, 3
		'Response.Write "SQL75="&SQL75&"<BR>"
		Do While NOT oRs75.EOF 		
            TheJobNumber=oRs75("rf_fh_id")
			Set oConn = Server.CreateObject("ADODB.Connection")
			oConn.ConnectionTimeout = 100
			oConn.Provider = "MSDASQL"
			oConn.Open DATABASE		
		    ...
		    If trim(Tempdestination)=Trim(FinalDestination) or trim(TempOrigination)="72" then
		        ...
		        oConn.Execute(L_SQL_44)	
						...
						Select Case LocationCode
							Case "CSSF"
							objMail.CC = "CSFGroup@Logisticorp.us"
                            'objMail.CC = "tracy.johnson@ti.com;m-haralson@ti.com;xscottsp@ti.com;rb44@ti.com;x0157297@ti.com;x0124281@ti.com;x0187225@ti.com"
                ...
                If TempOrigination="HFABQC" then
		            L_SQL_44="PHONE_CHANGE_STATUS '" & TheJobNumber & "', '56', 'UNS', '', '',  '"& UserID &"', '"& UnitID &"'" 
                    else		        
...
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
                ...
                'Response.Write "Got here #1<br>"
					...
					oConn.Open DATABASE
					''''UPDATES THE WAFER
                    If trim(ErrorMessage)="" and Trim(TheBarCode)>" " then
                    'Response.Write "Got here #2<br>"
						l_cSQL = "UPDATE FCREFS SET ref_status = 'o'" 
						If PODID>"" then
						    l_cSQL = l_cSQL&", PUPOD = '"&PODID&"' "
					    End if
                        l_cSQL = l_cSQL&" WHERE (ref_status is NULL) AND (rf_box = '" &TheBarCode& "')"
					...
					oRs.ActiveConnection = DATABASE	
					''''''CHECKS TO SEE IF ITS OKAY TO CHANGE THE ORDER STATUS
					SQL = "SELECT rf_fh_id FROM fcrefs INNER JOIN fcfgthd ON fcrefs.rf_fh_id = fcfgthd.fh_id"
					SQL = SQL&" WHERE (rf_box='"&TheBarCode&"')"
					'''''SQL = SQL&" AND ((Ref_Status IS NULL) or (Ref_Status='Q'))"
					SQL = SQL&" AND (FH_Status='ACC') AND (Ref_Status='o')"
				    'Response.Write "SQL="&SQL&"<BR>"
					oRs.Open SQL, DATABASE, 1, 3
			
					Do While not oRs.EOF 
					    TheJobNumber=oRs("rf_fh_id")
