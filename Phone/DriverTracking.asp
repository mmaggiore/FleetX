...
<!-- #include file="LogoSection.asp" -->
    ...
    <tr><td><img src="images/pixel.gif" height="5" width="1" /></td></tr>
		<tr><td align="center"><input type="button" value="Back" name="ClickBack" onclick=(history.back())  id="gobutton"></td></tr>
        <tr><td><img src="images/pixel.gif" height="5" width="1" /></td></tr>
        <tr>
        ...
        <tr><td><img src="images/pixel.gif" height="5" width="1" /></td></tr>		
        <tr><td align="left" class="mainpagetext">
		...
		If Submit>"" then
			...
			Set RSEVENTS2 = Server.CreateObject("ADODB.Recordset")
				...
				l_csql = " SELECT fcfgthd.fh_id, fcfgthd.fh_bt_id, fclegs.fl_sf_rta, fclegs.fl_st_rta, fclegs.fl_firstdrop, fcfgthd.fh_status, fcfgthd.fh_ship_dt, fcfgthd.fh_user5, fclegs.fl_sf_id, "
                l_csql = l_csql&" fclegs.fl_sf_name, fclegs.fl_st_id, fclegs.fl_st_building, fclegs.fl_st_name, fclegs.fl_t_atp, fclegs.fl_t_atd, fclegs.fl_pod, fcfgthd.fh_custpo, fcfgthd.fh_priority, fcrefs.rf_ref, "
                l_csql = l_csql&" fclegs.fl_sf_building, fclegs.fl_sf_addr1, fclegs.fl_sf_addr2, fclegs.fl_sf_city, fclegs.fl_sf_state, fclegs.fl_sf_country, fclegs.fl_sf_zip, fclegs.fl_sf_clname, "
			...
			''''''''''''''''''''''''''''''''''''''''
				...
				Do while not RSEVENTS2.EOF 
                    ...
                    fl_sf_building = RSEVENTS2("fl_sf_building")
                    ...
                    fl_st_building = RSEVENTS2("fl_st_building")
		...
		%>
			<table cellpadding="3" cellspacing="0" width="300" border="1" align="left" ID="Table5">
				...
				<%If UsesLots=FALSE then
						ColspanNumber="7"
				%>
					<td class="FleetXRedSection" nowrap="nowrap" colspan="2">
						Document Number(s)
					</td>				
					<%else
						ColspanNumber="8"
	...
	<%end if%>		
				...
				<tr>
					...
					%>
						<td colspan="2" class="MainPageTextSmaller" nowrap valign="top">
							<%=fh_custpo%>
						</td>				
						<%else
						%>
			...
			Set RSEVENTS2 = Nothing
										...
										if NOT Recordset1.EOF then
                                            ...
                                            If trim(NumberOfPieces)>"" then
                                                Response.write "<tr><td colspan='2'><b>"&numberofpieces&" "&rf_box&"</b></td></tr>"
                                            End if
											Else
			...
			'Response.Write "ColspanNumber="&ColspanNumber&"<BR>"
					DisplayToLocation=trim(fl_st_id)
					DisplayFromLocation=trim(fl_sf_id)
					...
					End if										
            if (fh_status = "ARV" or fh_status = "AC2") and vehicleID=912780 then
              DisplayFromLocation = "SRHUB"
             end if           
					'''REMOVED ON 2/21/11 If (Trim(fl_st_id)="55" AND (Trim(VehicleID)<>"611" AND trim(VehicleID)<>"612" AND trim(VehicleID)<>"613" AND trim(VehicleID)<>"112" AND trim(VehicleID)<>"123")) or Trim(fl_st_id)="72" then
					...
					'''REMOVED ON 2/21/11 If (((Trim(fl_sf_id)="55" OR Trim(fl_sf_id)="TOPPAN" OR (Trim(fl_sf_id)="PHO")) AND Trim(fl_st_id)<>"TISHERMANRET") AND (Trim(VehicleID)="611" or trim(VehicleID)="612" or trim(VehicleID)="613" or trim(VehicleID)="112" or trim(VehicleID)="123")) then 
					'''REMOVED ON 2/21/11 	If trim(fl_st_id)<>"HFABRET" then
					'''REMOVED ON 2/21/11 		DisplayToLocation="SB-HUB" 
					...
					'''REMOVED ON 2/21/11 End if
					'''REMOVED ON 2/21/11 If ((Trim(fl_sf_id)="55" OR Trim(fl_sf_id)="TOPPAN" OR Trim(fl_sf_id)="PHO") AND (Trim(VehicleID)<>"611" AND trim(VehicleID)<>"612" AND trim(VehicleID)<>"613" AND trim(VehicleID)<>"112" AND trim(VehicleID)<>"123")) then 
					'''REMOVED ON 2/21/11 	DisplayFromLocation="SB-HUB" 
					'''REMOVED ON 2/21/11 	FromName="South Building HUB"
					...
					''''''''''''''''''''''''''''''''''
						...
						'Response.Write "fl_st_id="&fl_st_id&"<BR>"
							'''REMOVED ON 2/21/11 If trim(VehicleID)="123" and trim(fl_st_id)="TISHERMA" then
							'''REMOVED ON 2/21/11 	'REsponse.Write "GOT HERE!<BR>"
							'''REMOVED ON 2/21/11 	DisplayToLocation="SB-HUB"
							'''REMOVED ON 2/21/11 End if
							'''REMOVED ON 2/21/11 If trim(VehicleID)="613" and trim(fl_st_id)="TISHERMA" and Trim(Fl_sf_id)<>"CSSF" then
							'''REMOVED ON 2/21/11 	'REsponse.Write "GOT HERE!<BR>"
							'''REMOVED ON 2/21/11 	DisplayFromLocation="SB-HUB"
							'''REMOVED ON 2/21/11 End if												
					''''''''''''''''''''''''''''''''''								
	...
	%>
			...
			From:<br>
					...
					If (Trim(fl_sf_id)="PHO" AND Trim(fl_st_id)="TISHERMANRET") THEN
						fl_st_addr1=""
					End if
					%>
					<!--
