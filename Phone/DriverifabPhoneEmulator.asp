...
%>
        ...
        'response.write "GOT HERE????<BR>"
			Set Recordset1 = Server.CreateObject("ADODB.Recordset")
			Recordset1.ActiveConnection = DATABASE
			Recordset1.Source = "SELECT st_id, CompanyName, CompanyAddress, CompanyBuilding, CompanySuite, CompanyCity FROM PreExistingCompanies WHERE (st_alias='"&AliasCode&"')"
			...
			If Recordset1.eof then
				...
				'objMail.cc = "4692269939@tmomail.net;htmlmale@yahoo.com"
				'objMail.cc = "4692269939@tmomail.net;pcurrin@ti.com"
				varSubject = "Bogus Location Code"
				'objMail.MailFormat = cdoMailFormatMIME
				'objMail.BodyFormat = cdoBodyFormatHTML
				'objMail.Body = Body
				'objMail.Send
        ...
        If OtherBillToID="80" then
			...
			'response.write "289 m=" & m & ",LocationCode=" & LocationCode & "<br>"
      If m>0 and trim(LocationCode) <> "SRHUB" then
...
'-------------------STARTS THE DROP OFF	
				...
				SQL = "SELECT fh_bt_id, Fl_SF_ID, Fh_ID, fh_User5, Fl_ST_ID, fl_sf_comment, FH_Status, Fh_Priority FROM fcfgthd INNER JOIN fclegs ON fcfgthd.fh_id = fclegs.fl_fh_id"
				SQL = SQL&" WHERE (fh_ship_dt>'"&now()-30&"') AND ((FL_Leg_Status='c') OR (FL_Leg_Status is NULL)) AND (fl_un_id='"&VehicleID&"') "

        'if location is SRHUB, show only ONB orders regardless of destination, but only stockroom/91
        if LocationCode <> "SRHUB" and LocationCode <> "D6N1B" and LocationCode <> "DOCK7" then
          SQL = SQL&" and (" & varToLocations & ") "
				end if
        
        'if location is D6N1B show only D6W3 or D5N2 destinations for stockroom/91
        Select Case LocationCode
          Case "SRHUB"
            SQL = SQL&" AND (fh_status='ONB') AND (fh_bt_id=91)"
          Case "D6N1B"
            SQL = SQL&" AND (fh_status='ONB') AND (fh_bt_id=91) AND (fl_st_id='D6W3' or fl_st_id='D6N2')"
          Case "DOCK7"
            SQL = SQL&" AND (fh_status='ONB') AND (fh_bt_id=91) AND (fl_st_id='DM5M' or fl_st_id='DM5Q' or fl_st_id='DM5S3')"
          Case Else
        ...
        End Select
				...
				'response.write "422 DROP OFF SQL="&SQL&"<BR>"
				...
				%>
					<table width="300" cellpadding="0" cellspacing="0" border="0" bordercolor="green" align="left" ID="Table1">
						...
						<tr>
							<td class="mainpagetextbold" colspan="3" align="center">
                                ...
                                <%
                                    ...
                                    End if
                                    If trim(CompanyAddress)>"" then
                                        Response.write CompanyAddress&"<br />"
                                    End if
                                    If trim(CompanyBuilding)>"" then
                                        Response.write CompanyBuilding&"<br />"
                                    End if
                                    If trim(CompanySuite)>"" then
                                        Response.write CompanySuite&"<br />"
                                    End if
                                    If trim(CompanyCity)>"" then
                                        Response.write CompanyCity
                                    End if
                                     %>
							</td>
						</tr>						
                        ...
                        <tr><td><img src="images/pixel.gif" height="5" width="1" /></td></tr>
				<%
				'check if material handler is logged in before dropping at SRHUB
        SRHUBLogin = "Y"
        If LocationCode = "SRHUB" then
                Set RecordsetB1 = Server.CreateObject("ADODB.Recordset")
								RecordsetB1.ActiveConnection = DATABASE
								SQL_B1="SELECT * FROM AvailableVehicles WHERE (VehicleID = '912780') AND (AvailableStatus <> 'x')"
								RecordsetB1.Source = SQL_B1
								RecordsetB1.CursorType = 0
								RecordsetB1.CursorLocation = 2
								RecordsetB1.LockType = 1
								RecordsetB1.Open()
								RecordsetB1_numRows = 0
								if RecordsetB1.EOF then
                  SRHUBLogin = "N"
								End if
								RecordsetB1.Close()
								Set RecordsetB1 = Nothing					
        End If
          
        If not oRs.EOF and SRHUBLogin="Y" then
					%>
						<tr>
							<td align="center">&nbsp;</td>						
						...
						CloseTable="y"
						ELSE
						  if SRHUBLogin="N" then
                Response.Write "<tr><td colspan='4' align='center'>No Dropoff to SRHUB allowed - No Material Handler is available.</td></tr><tr><td>&nbsp;</td></tr>"
						  else
				...
				If (oRS.EOF) OR SRHUBLogin = "N" then
					sendback1="y"
				End if
				If NOT (oRS.BOF AND oRS.EOF) AND SRHUBLogin = "Y" Then
					...
					If JobNumber<>TempJobNumber then
						...
						Select Case JobStatus
							...
							Case Else
								...
								<%
						End Select
						Response.Write "<td valign='top' nowrap><font color='"&PriorityColor&"'>"&Y&") <a href='DriverTracking.asp?JobNumber="&JobNumber&"&fh_bt_id="&fh_bt_id&"'><font color='"&PriorityColor&"'>"&JobNumber&"</font></a></font>"
  										Set Recordset1 = Server.CreateObject("ADODB.Recordset")
										Recordset1.ActiveConnection = DATABASE
...
'-------------------STARTS THE PICK UP	
        ...
        if LocationCode <> "SRHUB" then 
            ...
            SQL = SQL&" AND ((fh_status='ACC')"
					...
					if LocationCode = "SRHUB" then
          	SQL = SQL&" AND ((fh_status='AC2') AND (fl_secacc is not null))"
          else
            'response.write "895 here<br>"
						if VehicleID=912780 then
              'response.write "897 dont show AC2 orders<br>"
            else
            ...
            SQL = SQL&") "
				...
				End if
				SQL = SQL&" ORDER BY fh_priority, fh_id"
				'Response.write "903 vehicleID="&vehicleID&",Location="&LocationCode&"<BR>"
				'response.write "904 PICK UP SQL="&SQL&"<BR>"
            ...
            If trim(Android)="y" or  trim(IDCookies)="14141414" or ourphone="y" or trim(IDCookies)="22"  then %>
            ...
            'Response.write " trim(Android)="&Android&"<BR>"
            'Response.write " trim(IDCookies)="&IDCookies&"<BR>" 
             'Response.write " ourphone="&ourphone&"<BR>"
