    ...
    <table  border=1 Align='center' cellpadding="0" cellspacing="0" width="850">
        ...
        <TR>
         ...
         <TR>
            ...
            <%
		        l_cSQL = "SELECT rf_ref, numberofpieces " & _
				"FROM fcrefs " & _
		...
		IF NOT oRefs.EOF THEN
			DO WHILE NOT oRefs.EOF
                ...
                numberofpieces=oRefs.Fields("numberofpieces")
                'Response.write "numberofpieces="&numberofpieces&"<BR>"

				oRefs.MoveNext
			LOOP
		END IF
		set oRefs = nothing	
        If trim(l_cPieces)="" or isnull(l_cPieces) then
            'Response.write "got here!<BR>"
             l_cPieces=numberofpieces  
        End if
        'Response.write "TEST..."
                %>
                <TR>
